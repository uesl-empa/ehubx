...
# Literals
LOG_MODULE_STR: str = "pars/ebm_tech"
...
def parse_primary(
    ...
    for tech_node in techs_node:
        _parse_ebm_tech_primary(tech_node, stages, ecs, techs, ebm_techs)
    return ebm_techs
...
def _parse_ebm_tech_primary(
    ...
    # cap_unit
    cap_unit = ecs.get_unit(ec_id)
    techs.set_cap_unit(tech_id, cap_unit)
    # in_eff
    in_eff = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_INEFF, stages, expected_unit=DimlessUnit()
    )
    if in_eff is not None:
        for stage_id, value in in_eff.items():
            ebm_techs.set_in_eff(stage_id, tech_id, value)
    # out_eff
    out_eff = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_OUTEFF, stages, expected_unit=DimlessUnit()
    )
    if out_eff is not None:
        for stage_id, value in out_eff.items():
            ebm_techs.set_out_eff(stage_id, tech_id, value)
    # standby_loss
    standby_loss = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node,
        YAMLKEY_STANDBYLOSS,
        stages,
    ...
    # soc_min
    soc_min = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_SOCMIN, stages, expected_unit=DimlessUnit()
    )
    if soc_min is not None:
        for stage_id, value in soc_min.items():
            ebm_techs.set_soc_min(stage_id, tech_id, value)
    # soc_max
    soc_max = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_SOCMAX, stages, expected_unit=DimlessUnit()
    )
    ...
    storage_cap = yaml_parser.parse_mandatory_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_STORAGECAP, stages, expected_unit=ec_unit
    )
    for stage_id, value in storage_cap.items():
        ebm_techs.set_storage_cap(stage_id, tech_id, value)
    # charge_max
    charge_max = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_CHARGEMAX, stages, expected_unit=(ec_unit / TimeUnit.H)
    )
    if charge_max is not None:
        for stage_id, value in charge_max.items():
            ebm_techs.set_charge_max(stage_id, tech_id, value)
    # discharge_max
    discharge_max = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node,
        YAMLKEY_DISCHARGEMAX,
        stages,
    ...
    # discharge_control
    discharge_control = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_DISCHARGECONTROL, stages, expected_unit=DimlessUnit()
    )
...
def parse_secondary(
    ...
    for hub_node in hubs_node:
        _parse_hub_secondary(hub_node, stages, ecs, ebm_techs)


def _parse_hub_secondary(
    hub_node: yaml_parser.YamlDictNode, stages: Stages, ecs: Ecs, ebm_techs: EbmTechs
) -> None:
    # id
    ...
    for tech_id in ebm_techs.ids:
        ...
        if tech_node is None:
            continue
        _parse_tech_secondary(tech_node, hub_id, tech_id, stages, ecs, ebm_techs)
...
def _parse_tech_secondary(
    ...
    if num_vehicles is not None:
        for stage_id, num_veh in num_vehicles.items():
            ebm_techs.set_num_vehicles(stage_id, hub_id, tech_id, num_veh)
    # soc_init
    soc_init = yaml_parser.parse_optional_value_from_dict_node(
        ebm_params_node, YAMLKEY_SOCINIT, expected_unit=DimlessUnit()
    )
    if soc_init is not None:
        ebm_techs.set_soc_init(hub_id, tech_id, soc_init)
    # demand_modifier
    demand_modifier = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_DEMANDMODIFIER, stages, expected_unit=DimlessUnit()
    )
    ...
    # demand_nominal
    ec_unit = ecs.get_unit(ebm_techs.get_ec(tech_id))
    demand_nominal = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node,
    ...
    availability = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ebm_params_node, YAMLKEY_AVAILABILITY, stages, expected_unit=DimlessUnit()
    )
    if availability is not None:
        for stage_id, value in availability.items():
            ebm_techs.set_availability_def(stage_id, hub_id, tech_id, value)
    # profiles
    _parse_tech_secondary_profiles(ebm_params_node, hub_id, tech_id, ec_unit, ebm_techs)
        ...
        for s, h, x, key in df.columns:
            ...
            try:
                ...
                raise exceptions.ParsingException(
                    profile_path,
                    f"Invalid unit '{ex.unit}' for EBM tech profile key '{key}' "
                    f"for (stage, hub, tec) tuple ({s}, {h}, {x})",
                    module=LOG_MODULE_STR,
                ) from ex

            expected_unit: Unit
            check_unit: bool = True
            if key == YAMLKEY_DEMANDNOMINAL:
                expected_unit = ec_unit / TimeUnit.H
            elif key == YAMLKEY_AVAILABILITY:
            ...
            if check_unit and not unit.same_type_as(expected_unit):
                raise exceptions.ParsingException(
                    profile_path,
                    f"Invalid unit '{unit}' for EBM tech profile key '{key}' "
