...
YAMLKEY_HEURMAX = "heur_max"
YAMLKEY_HEURSUMMAX = "heur_sum_max"
...
def parse(basic_subpath: str) -> Tuple[Ecs, Optional[yaml_parser.YamlNode]]:
    ecs = Ecs()
...
def _parse_ec(ec_node: yaml_parser.YamlDictNode, ecs: Ecs) -> None:
    ...
    # is_energy
    is_energy = yaml_parser.parse_optional_bool_from_dict_node(
        ec_node, YAMLKEY_ISENERGY
    )
    if is_energy is not None:
        ecs.set_is_energy(ec_id, is_energy)
    # imp_exp_type
    imp_exp_type = yaml_parser.parse_optional_str_from_dict_node(
    ...
    if imp_exp_type is not None:
        if imp_exp_type == YAMLKEY_IMPEXPTYPE_CROSS:
            ...
            invalidity_reason = f"{imp_exp_type} is not a known import-export-type"
            raise exceptions.InvalidValueException(
                ec_node.file_path,
                node_path_str,
                invalidity_reason,
    ...
    # heur_max
    heuristic_max = yaml_parser.parse_optional_value_from_dict_node(
        ec_node, YAMLKEY_HEURMAX, expected_unit=unit / TimeUnit.H
    )
    if heuristic_max is not None:
        ecs.set_heuristic_max(ec_id, heuristic_max)
    # heur_sum_max
    heuristic_sum_max = yaml_parser.parse_optional_value_from_dict_node(
        ec_node, YAMLKEY_HEURSUMMAX, expected_unit=unit
    )
    if heuristic_sum_max is not None:
        ecs.set_heuristic_sum_max(ec_id, heuristic_sum_max)
