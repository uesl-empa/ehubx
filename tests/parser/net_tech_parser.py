...
from ehubx.parser import exceptions, net_link_parser, yaml_parser
...
YAMLKEY_TRL = "trl"
YAMLKEY_TRANSDECAY = "trans_decay"
...
def parse_primary(
    network_subpath: str, energy_system: EnergySystem, stages: Stages, ecs: Ecs
) -> NetworkTechs:
    ...
    for net_tech_node in net_techs_node:
        _parse_net_tech_primary(net_tech_node, energy_system, stages, ecs, net_techs)
    # Logging
    _log(net_techs)
    # Return
    return net_techs
...
def _parse_net_tech_primary(
    net_tech_node: yaml_parser.YamlDictNode,
    ...
    # net_tech_id
    net_tech_id_str = yaml_parser.parse_mandatory_str_from_dict_node(
        net_tech_node, YAMLKEY_NETTECHID
    )
    net_tech_id = NetTechId(net_tech_id_str)
    net_techs.add_id(net_tech_id)
    # ec
    ec = EcId(yaml_parser.parse_mandatory_str_from_dict_node(net_tech_node, YAMLKEY_EC))
    net_techs.set_ec(net_tech_id, ec, ecs.get_unit(ec))
    # lifetime
    lifetime = yaml_parser.parse_mandatory_value_from_dict_node(
        net_tech_node, YAMLKEY_LIFETIME, expected_unit=TimeUnit.A
    )
    ...
    trl_dict = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        net_tech_node, YAMLKEY_TRL, stages, expected_unit=DimlessUnit()
    )
    if trl_dict is None:
    ...
    if trl_dict is not None:
        for stage_id, trl in trl_dict.items():
            ...
            net_techs.add_allowed_stage(stage_id, net_tech_id)
    # trans_decay
    trans_decay = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        net_tech_node,
        YAMLKEY_TRANSDECAY,
        stages,
        expected_unit=(DimlessUnit() / LengthUnit.M),
    )
    if trans_decay is not None:
        for stage_id, value in trans_decay.items():
            net_techs.set_trans_decay(stage_id, net_tech_id, value)
    # unit_cap_min
    unit_cap_min = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        net_tech_node,
        YAMLKEY_UNITCAPMIN,
        stages,
        expected_unit=(ecs.get_unit(ec) / TimeUnit.H),
    )
    if unit_cap_min is not None:
        for stage_id, value in unit_cap_min.items():
            net_techs.set_unit_cap_min(stage_id, net_tech_id, value)
    # costs
    _parse_costs(net_tech_node, net_tech_id, energy_system, stages, ecs, net_techs)
    # emissions
    _parse_emissions(net_tech_node, net_tech_id, stages, ecs, net_techs)


def _parse_costs(
    net_tech_node: yaml_parser.YamlDictNode,
    net_tech_id: NetTechId,
    energy_system: EnergySystem,
    stages: Stages,
    ecs: Ecs,
    net_techs: NetworkTechs,
    ...
    if costs_node is None:
        net_techs.set_interest_rate(net_tech_id, energy_system.interest_rate_def)
        return
    ...
    capex_per_cap = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ...
        expected_unit=(
            CurrencyUnit.CHF
            / (
                (ecs.get_unit(net_techs.get_ec(net_tech_id)) / TimeUnit.H)
                * LengthUnit.M
            )
        ),
    )
    ...
    opex_per_energy = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        ...
        expected_unit=(
            CurrencyUnit.CHF
            / (ecs.get_unit(net_techs.get_ec(net_tech_id)) * LengthUnit.M)
        ),
    )
...
def _parse_emissions(
    ...
    # co2_per_energy
    co2_per_energy = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        emissions_node,
        YAMLKEY_CO2PERENERGY,
        stages,
        expected_unit=(
            MassUnit.KG / (ecs.get_unit(net_techs.get_ec(net_tech_id)) * LengthUnit.M)
        ),
    )
...
def parse_secondary(
    link_root_node: Optional[yaml_parser.YamlNode], ecs: Ecs, net_techs: NetworkTechs
) -> None:
    ...
    for start_hub_node in start_hubs_node:
        ...
        for end_hub_node in end_hubs_node:
            ...
            for link_node in links_node:
                _parse_link_secondary(link_node, net_tech_lists, ecs, net_techs)


def _parse_net_tech_lists(
    link_root_node: yaml_parser.YamlNode,
) -> Dict[str, List[NetTechId]]:
    ...
    yaml_parser.check_node_type(net_tech_lists_node, yaml_parser.YamlNodeKind.LIST)
    net_tech_lists_node.set_id(YAMLKEY_NETTECHLISTID)
    for net_tech_list_node in net_tech_lists_node:
        net_tech_list_id = yaml_parser.parse_mandatory_str_from_dict_node(
        ...
        net_tech_lists[net_tech_list_id] = [
            NetTechId(net_tech) for net_tech in net_techs
        ]
    return net_tech_lists


def _parse_link_secondary(
    ...
    allowed_net_techs = _parse_allowed_net_techs(
    ...
    if net_tech_params_node is not None:
        yaml_parser.check_node_type(net_tech_params_node, yaml_parser.YamlNodeKind.LIST)
        net_tech_params_node.set_id(YAMLKEY_NETTECHID)
        for net_tech_node in net_tech_params_node:
            _parse_net_tech_secondary(
                net_tech_node, link_id, ecs, net_techs, allowed_net_techs
            )


def _parse_net_tech_secondary(
    net_tech_node: yaml_parser.YamlDictNode,
    link_id: NetLinkId,
    ecs: Ecs,
    net_techs: NetworkTechs,
    allowed_net_techs: Set[NetTechId],
) -> None:
    # net_tech_id
    net_tech_id_str = yaml_parser.parse_mandatory_str_from_dict_node(
        net_tech_node, YAMLKEY_NETTECHID
    ...
    net_tech_id = NetTechId(net_tech_id_str)
    ec_unit = ecs.get_unit(net_techs.get_ec(net_tech_id))
    # Warning: Specifying net_tech params if net_tech is not allowed
    if net_tech_id not in allowed_net_techs:
        msg = (
            f"Parsing irregularity in file {net_tech_node.file_path}: "
    ...
    cap_init = yaml_parser.parse_optional_value_from_dict_node(
        net_tech_node, YAMLKEY_CAPINIT, expected_unit=(ec_unit / TimeUnit.H)
    )
    ...
    age_init = yaml_parser.parse_optional_value_from_dict_node(
        net_tech_node, YAMLKEY_AGEINIT, expected_unit=TimeUnit.A
    )
...
def _parse_allowed_net_techs(
    ...
    for net_tech_list_id in allowed_net_tech_lists:
        if net_tech_list_id not in net_tech_lists:
            node_path_str = (
                link_node.node_path_as_str + "|" + YAMLKEY_ALLOWEDNETTECHLISTS
            )
            invalidity_reason = (
                f"{net_tech_list_id} is not a known " + f"{YAMLKEY_NETTECHLISTID}"
            )
...
def _log(net_techs: NetworkTechs) -> None:
    logging.log_file(
        f"Parsed {len(net_techs.ids)} network tech(s)", module=LOG_MODULE_STR
    )
    for n in net_techs.ids:
        logging.log_file(
            (
                f"  NetTech {n}: ec = {net_techs.get_ec(n)}, "
