...
def build(model: Model, system: EnergySystem) -> None:
    ...
    'Electricity-based mobility model' in the documentation.
    ...
    logging.log_file(
        (f"Built EBM tech module. Elapsed time: {int(elapsed.total_seconds())}s"),
...
def _build_base(model: Model, system: EnergySystem) -> None:
    ...
    setattr(
        model,
        SET_EBMTECH,
        Set(within=getattr(model, SET_TECH), initialize=[x.key for x in ebm_techs.ids]),
    )
    # [SET] EBM tech tuples
    setattr(
        model,
    ...
    setattr(
        ...
        PAR_EBMTECHEC,
        Param(
            getattr(model, SET_EBMTECH),
            within=Any,
            initialize={x.key: ebm_techs.get_ec(x).key for x in ebm_techs.ids},
        ),
    )
    ...
    setattr(
        ...
        VAR_EBMTECHINFLOW,
        Var(
            getattr(model, SET_EBMTECHTUPLE),
...
def _con_ebm_tech_cap(
    ...
    setattr(
        model,
        CON_EBMTECHCAP,
        Constraint(getattr(model, SET_EBMTECHTUPLE), rule=__rule_ebm_tech_cap),
    )


def _con_ebm_tech_inoutflow_max(
    model: Model,
    ecs: Ecs,
    ebm_techs: EbmTechs,
    mass_unit: MassUnit,
    ...
    # Time series
    availability = {
        (s, h, x): ebm_techs.get_availability(StageId(s), HubId(h), TechId(x))
    ...
    def __rule_ebm_tech_inflow_max(model, s, h, x, t):
        ...
        num_vehicles = ebm_techs.get_num_vehicles(StageId(s), HubId(h), TechId(x))
        availability_t = availability[s, h, x].get_value(TimeId(t)).to_float()
    ...
    def __rule_ebm_tech_outflow_max(model, s, h, x, t):
        ...
        num_vehicles = ebm_techs.get_num_vehicles(StageId(s), HubId(h), TechId(x))
        availability_t = availability[s, h, x].get_value(TimeId(t)).to_float()
        discharge_control = ebm_techs.get_discharge_control(
            StageId(s), TechId(x)
        ).to_float()
        ...
        if num_vehicles > 0 and availability_t > 0 and discharge_control > 0:
            outflow_max = (
                discharge_max * num_vehicles * availability_t * discharge_control
            )
        # Set constraint
...
def _con_ebm_tech_used(
    ...
    setattr(
        model,
        CON_EBMTECHUSED,
        Constraint(getattr(model, SET_EBMTECHTUPLE), rule=__rule_ebm_tech_used),
    )


def _con_ebm_tech_charging_dynamic(
    model: Model,
    ecs: Ecs,
    ebm_techs: EbmTechs,
    times: Times,
    ...
    consumption = {
        (s, h, x): ebm_techs.get_consumption(StageId(s), HubId(h), TechId(x), times)
        for (s, h, x) in getattr(model, SET_EBMTECHTUPLE)
    }
    ...
    def __rule_ebm_tech_charging_dynamic(model, s, h, x, t_hor):
        ...
        in_eff = ebm_techs.get_in_eff(StageId(s), TechId(x)).to_float()
        out_eff = ebm_techs.get_out_eff(StageId(s), TechId(x)).to_float()
        standby_loss = ebm_techs.get_standby_loss(StageId(s), TechId(x)).to_float(
        ...
        energy_flow = (
            in_eff * getattr(model, VAR_EBMTECHINFLOW)[s, h, x, t]
            - getattr(model, VAR_EBMTECHOUTFLOW)[s, h, x, t] / out_eff
        )
        # Calculate energy level at next horizon_ts
        energy_next = (
            (1 - standby_loss) * getattr(model, VAR_EBMTECHENERGY)[s, h, x, t_hor]
            + energy_flow
            - consumption_t
        )
        ...
        # Set the constraint
        return getattr(model, VAR_EBMTECHENERGY)[s, h, x, t_hor_next] == energy_next

    setattr(
        model,
        CON_EBMTECHCHARGINGDYNAMIC,
        Constraint(
            getattr(model, SET_EBMTECHTUPLE),
            getattr(model, SET_TIMEHORIZON),
...
def _con_ebm_tech_energy_minmax(
    model: Model,
    ecs: Ecs,
    ebm_techs: EbmTechs,
    mass_unit: MassUnit,
...
def _con_ebm_tech_energy_init(
    ...
    def __rule_ebm_tech_energy_init(model, s, h, x):
        ...
        if StageId(s) != s_0:
            return (
                getattr(model, VAR_EBMTECHENERGY)[s, h, x, t_hor_0]
                == getattr(model, VAR_EBMTECHENERGY)[s_0.key, h, x, t_hor_0]
            )
    ...
    setattr(
        model,
        CON_EBMTECHENERGYINIT,
        Constraint(getattr(model, SET_EBMTECHTUPLE), rule=__rule_ebm_tech_energy_init),
    )
