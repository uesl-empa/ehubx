...
CON_HPTECHCAPAVAIL: str = "C_HpTechCapAvailability"
"""Name of constraint respecting the tech capacity and the availability
for heat pumps"""
...
def _build_base(
    ...
    hp_techs: HeatpumpTechs = system.hp_techs
    ...
    _con_hp_tech_used(model, system)
...
def _con_hp_tech_cop(model: Model, hp_techs: HeatpumpTechs, times: Times) -> None:
    ...
    def __rule_hp_tech_cop_ht(model, s, h, x, t):
        # Get parameters
        cop = cops[s, h, x].get_value(TimeId(t)).to_float()
        ec_ht_out = hp_techs.get_ec_ht_out(TechId(x))
        # Get component powers for cooling mode
        pow_cond = getattr(model, VAR_HPTECHOUT)[s, h, x, ec_ht_out.key, t]
        pow_elec = getattr(model, VAR_HPTECHELECHT)[s, h, x, t]
...
def _con_hp_tech_cap(model: Model, hp_techs: HeatpumpTechs) -> None:
    def __rule_hp_tech_cap(model, s, h, x, t):
        ...
        ec_co_in = hp_techs.get_ec_co_in(TechId(x))
        availability = (
            hp_techs.get_availability(StageId(s), HubId(h), TechId(x))
            .get_value(TimeId(t))
            .to_float()
        )
        ...
        # Set constraint
        return pow_cond <= availability * getattr(model, VAR_TECHCAP)[s, h, x]
...
def _con_hp_tech_used(model: Model, system: EnergySystem) -> None:
    # Extract data from modules
    ecs = system.ecs
    hp_techs = system.hp_techs
    times = system.times
    ...
    def __rule_hp_tech_used(model, s, h, x):
        ...
        bigm = bigm_cap
        # b) bigM by heuristic limit
        max_sum_out_ht = system.get_heur_limit_max_sum_out(
            StageId(s), HubId(h), ec_ht_out
        ).to_float(unit=ec_unit)
        max_sum_in_co = system.get_heur_limit_max_sum_in(
            StageId(s), HubId(h), ec_co_in
        ).to_float(unit=ec_unit)
        bigm_heur = max_sum_out_ht + max_sum_in_co
