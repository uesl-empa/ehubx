...
class UnitException(EhubXException):
    """
    Exception that inherits from the main ehubX exception and can be
    thrown from the data module if a unit error occurs. In addition to its
    EhubXException properties, it contains a unit string which can be used to
    identify its origin.
    """

    def __init__(self, unit: str, msg: str, module: str = ""):
        self.unit = unit
        super().__init__(msg, module=module)


class ValueException(EhubXException):
    """
    Exception that inherits from the main ehubX exception and can be
    thrown from the data module if a value error occurs. In addition to its
    EhubXException properties, it contains a value string which can be used to
    identify its origin.
    """

    def __init__(self, value: str, msg: str, reason: str = "", module: str = ""):
        self.value = value
        self.reason = reason
        super().__init__(msg, module=module)
