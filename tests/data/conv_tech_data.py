...
class ConversionTechs:
    ...
    def remove_id(self, x: TechId) -> None:
        ...
        if x in self._in_ecs:
            self._in_ecs.pop(x)
        for e in list(self._ec_unit.keys()):
            self._ec_unit.pop(e)
    ...
    # ------------------------- #
    def get_opex_per_energy(self, s: StageId, x: TechId) -> Value:
        """
        ...
        :type x: TechId
        :return: opex_per_energy
        :rtype: Value
        """
        self._check_id(x, ExceptionKey.OPEXPERENERGY_GET)
        unit = CurrencyUnit.CHF / self._ec_unit[self.get_out_ec_main(x)]
        return self._opex_per_energy.get((s, x), Value(DEF_OPEXPERENERGY, unit))

    def set_opex_per_energy(
        self, s: StageId, x: TechId, opex_per_energy: Value
    ) -> None:
        """
    ...
    def add_in_ec(self, x: TechId, e: EcId, ec_unit: Unit) -> None:
        """
    ...
    # Property: in_part #
    # ----------------- #
    def get_in_part(self, s: StageId, x: TechId, e: EcId) -> Value:
        """
    ...
    def set_in_part(self, s: StageId, x: TechId, e: EcId, in_part: Value) -> None:
        """
        ...
        :param e: Id of input ec
        :type e: EcId
        :param in_part: Input portion
        :type in_part: Value
        """
        self._check_id(x, ExceptionKey.INPART_SET)
        self._check_in_ec(x, e, ExceptionKey.INPART_SET)
        expected_unit = self._ec_unit[e]
        if not in_part.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.INPART_SET.value,
                [s, x, e],
                f"Unit of in_part[{s}, {x}, {e}] = {in_part} "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    def add_out_ec(self, x: TechId, e: EcId, ec_unit: Unit) -> None:
        """
    ...
    def set_out_ec_main(self, x: TechId, e: EcId) -> None:
        ...
        self._check_out_ec(x, e, ExceptionKey.OUTECMAIN_SET)
    ...
    def get_out_eff(self, s: StageId, x: TechId, e: EcId) -> TimeSeries:
        ...
        technology's main input ec. This is a mandatory parameter.
        ...
        :type e: EcId
        :return: Output efficiencies
        :rtype: TimeSeries
        """
        self._check_id(x, ExceptionKey.OUTEFF_GET)
        self._check_out_ec(x, e, ExceptionKey.OUTEFF_GET)
        if (s, x, e) not in self._out_eff:
            raise exceptions.DataException(
                ExceptionKey.OUTEFF_GET.value,
                [s, x, e],
                f"Unknown ({s.kind_as_str}, {x.kind_as_str}, "
    ...
    def set_out_eff(
        self, s: StageId, x: TechId, e: EcId, t: TimeId, out_eff: Value
    ) -> None:
        """
        ...
        self._check_out_ec(x, e, ExceptionKey.OUTEFF_SET)
        in_ec_main = self.get_in_ec_main(x)
        expected_unit = self._ec_unit[e] / self._ec_unit[in_ec_main]
        if not out_eff.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.OUTEFF_SET.value,
                [s, x, e],
                f"Unit of out_eff[{s}, {x}, {e}] = {out_eff} "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
        if (s, x, e) not in self._out_eff:
    ...
    def set_out_eff_def(
        self, s: StageId, x: TechId, e: EcId, out_eff_def: Value
    ) -> None:
        """
        ...
        output of that ec relative to the technology's main input ec. This
        ...
        :param e: Id of output ec
        :type e: EcId
        :param out_eff_def: Default output efficiency
        :type out_eff_def: Value
        """
        self._check_id(x, ExceptionKey.OUTEFF_SET)
        self._check_out_ec(x, e, ExceptionKey.OUTEFF_SET)
        in_ec_main = self.get_in_ec_main(x)
        expected_unit = self._ec_unit[e] / self._ec_unit[in_ec_main]
        if not out_eff_def.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.OUTEFF_SET.value,
                [s, x, e],
                f"Unit of out_eff[{s}, {x}, {e}] = {out_eff_def} "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
        if (s, x, e) not in self._out_eff:
    ...
    # --------------------- #
    def get_out_sum_min(self, s: StageId, h: HubId, x: TechId) -> Value:
        """
        ...
        :param x: Id of conversion technology
        :type x: TechId
        :return: Minimal output on horizon
        :rtype: Value
        """
        self._check_id(x, ExceptionKey.OUTSUMMIN_GET)
        unit = self._ec_unit[self.get_out_ec_main(x)]
        return self._out_sum_min.get((s, h, x), Value(DEF_OUTSUMMIN, unit))

    def set_out_sum_min(
        self, s: StageId, h: HubId, x: TechId, out_sum_min: Value
    ) -> None:
        """
        ...
        :param x: Id of conversion technology
        :type x: TechId
        :param out_sum_min: Minimal output on horizon
        :type out_sum_min: Value
        """
        self._check_id(x, ExceptionKey.OUTSUMMIN_SET)
        expected_unit = self._ec_unit[self.get_out_ec_main(x)]
        if not out_sum_min.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.OUTSUMMIN_SET.value,
                [s, h, x],
                f"Unit of out_sum_min[{s}, {h}, {x}] = {out_sum_min} "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    # --------------------- #
    def get_out_sum_max(self, s: StageId, h: HubId, x: TechId) -> Value:
        """
        ...
        :param x: Id of conversion technology
        :type x: TechId
        :return: Maximal output on horizon
        :rtype: Value
        """
        self._check_id(x, ExceptionKey.OUTSUMMAX_GET)
        unit = self._ec_unit[self.get_out_ec_main(x)]
        return self._out_sum_max.get((s, h, x), Value(DEF_OUTSUMMAX, unit))

    def set_out_sum_max(
        self, s: StageId, h: HubId, x: TechId, out_sum_max: Value
    ) -> None:
        """
        ...
        :param x: Id of conversion technology
        :type x: TechId
        :param out_sum_max: Maximal output on horizon
        :type out_sum_max: Value
        """
        self._check_id(x, ExceptionKey.OUTSUMMAX_SET)
        expected_unit = self._ec_unit[self.get_out_ec_main(x)]
        if not out_sum_max.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.OUTSUMMAX_SET.value,
                [s, h, x],
                f"Unit of out_sum_max[{s}, {h}, {x}] = {out_sum_max} "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    def set_availability_def(
        ...
        :param x: Id of conversion technology
        :type x: TechId
        :param availability_def: Default value of availability
        ...
        if not availability_def.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ...
                [s, h, x],
                f"Unit of availability[{s}, {h}, {x}] = {availability_def} "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    def set_time_series_val(
        ...
        if kind == TimeSeriesKind.CONVTECHOUTEFF:
            x = TechId(ids[0])
            e = EcId(ids[1])
            unit = Unit.get_def_unit(
                self._ec_unit[e] / self._ec_unit[self.get_in_ec_main(x)]
            )
            self.set_out_eff(s, x, e, t, Value(value, unit=unit))
    ...
    def __init__(self) -> None:
        ...
        self._in_part: Dict[Tuple[StageId, TechId, EcId], Value] = {}
        ...
        self._opex_per_energy: Dict[Tuple[StageId, TechId], Value] = {}
    ...
    def _validate_in_part(self, stages: Stages, techs: Techs) -> None:
        ...
        for (s, x, e), in_part in self._in_part.items():
            ...
            if in_part.is_negative:
                msg = f"{in_part} = in_part[{s}, {x}, {e}] < 0"
                raise exceptions.DataException(
                    exc_key, [s, x, e], msg, module=LOG_MODULE_STR
                )
        # At least one in_part must be larger than zero
        for x in self.ids:
            for s in techs.get_allowed_stages(x):
                all_in_parts_zero = True
                for e in self.get_in_ecs(x):
                    if self.get_in_part(s, x, e).is_positive:
                        all_in_parts_zero = False
                        break
                if all_in_parts_zero:
                    msg = f"All in_parts for stage {s} and conv_tech {x} are zero"
                    raise exceptions.DataException(
    ...
    def _validate_out_eff(self, stages: Stages, times: Times) -> None:
        ...
        for (s, x, e), out_eff in self._out_eff.items():
            ...
            if out_eff.has_values:
                ...
                for t in times.ids:
                    if out_eff.get_value(t).is_negative:
                        msg = (
    ...
    # Ec checkers #
    # ----------- #
    def _check_in_ec(self, x: TechId, e: EcId, where: ExceptionKey) -> None:
        if e not in self._in_ecs[x]:
            raise exceptions.DataException(
                where.value,
                [x, e],
                f"Encountered ec {e} which is not registered as an input ec of "
                f"the conversion tech {x}. This happened while {where.value}",
                module=LOG_MODULE_STR,
            )

    def _check_out_ec(self, x: TechId, e: EcId, where: ExceptionKey) -> None:
        if e not in self._out_ecs[x]:
            raise exceptions.DataException(
                where.value,
                [x, e],
                f"Encountered ec {e} which is not registered as an output ec of "
                f"the conversion tech {x}. This happened while {where.value}",
                module=LOG_MODULE_STR,
            )
...
def copy_over_conv_tech(
    ...
    for e in src.get_in_ecs(x):
        tar.add_in_ec(x, e, src._ec_unit[e])
    ...
    for e in src.get_out_ecs(x):
        tar.add_out_ec(x, e, src._ec_unit[e])
