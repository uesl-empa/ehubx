...
class NetworkLinks:
    ...
    def add_ec(self, li: NetLinkId, e: EcId, ec_unit: Unit) -> None:
        """
        ...
        if li not in self._ecs:
            self._ecs[li] = set()
        self._ecs[li].add(e)
        self._cap_min_def[li, e] = Value(DEF_CAPMIN, unit=(ec_unit / TimeUnit.H))
        self._cap_max_def[li, e] = Value(DEF_CAPMAX, unit=(ec_unit / TimeUnit.H))
        self._sum_min_def[li, e] = Value(DEF_SUMMIN, unit=ec_unit)
        self._sum_max_def[li, e] = Value(DEF_SUMMAX, unit=ec_unit)
    ...
    # Property: length #
    # ---------------- #
    def get_length(self, li: NetLinkId) -> Value:
        """
    ...
    def set_length(self, li: NetLinkId, length: Value) -> None:
        """
        ...
        if not length.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.LENGTH_SET.value,
                [li],
                f"Unit {length.unit} of length[{li}] "
    ...
    # Property: cap_min #
    # ----------------- #
    def get_cap_min(self, s: StageId, li: NetLinkId, e: EcId) -> Value:
        """
        ...
        self._check_ec(li, e, ExceptionKey.CAPMIN_GET)
        return self._cap_min.get((s, li, e), self._cap_min_def[li, e])

    def set_cap_min(self, s: StageId, li: NetLinkId, e: EcId, cap_min: Value) -> None:
        """
        ...
        self._check_ec(li, e, ExceptionKey.CAPMIN_SET)
        expected_unit = self._cap_min_def[li, e].unit
        if not cap_min.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.CAPMIN_SET.value,
                [s, li, e],
                f"Unit {cap_min.unit} of cap_min[{s}, {li}, {e}] "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    # Property: cap_max #
    # ----------------- #
    def get_cap_max(self, s: StageId, li: NetLinkId, e: EcId) -> Value:
        """
        ...
        self._check_ec(li, e, ExceptionKey.CAPMAX_GET)
        return self._cap_max.get((s, li, e), self._cap_max_def[li, e])

    def set_cap_max(self, s: StageId, li: NetLinkId, e: EcId, cap_max: Value) -> None:
        """
        ...
        self._check_ec(li, e, ExceptionKey.CAPMAX_SET)
        expected_unit = self._cap_max_def[li, e].unit
        if not cap_max.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.CAPMAX_SET.value,
                [s, li, e],
                f"Unit {cap_max.unit} of cap_max[{s}, {li}, {e}] "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    def get_availability(self, s: StageId, li: NetLinkId, e: EcId) -> TimeSeries:
        ...
        :param e: Ec
        :type e: EcId
        :return: Time series for availability
        :rtype: TimeSeries
        """
        self._check_id(li, ExceptionKey.AVAILABILITY_GET)
        self._check_ec(li, e, ExceptionKey.AVAILABILITY_GET)
        if (s, li, e) not in self._availability:
            availability = TimeSeries()
            availability.def_value = Value(DEF_AVAILABILITY, unit=DimlessUnit())
            return availability
        return self._availability[s, li, e]

    def set_availability(
        self, s: StageId, li: NetLinkId, e: EcId, t: TimeId, availability: Value
    ) -> None:
        """
    ...
    def get_sum_min(
        ...
        :return: Minimal energy that has to be transported throughout the stage
        :rtype: Value
        """
        self._check_id(li, ExceptionKey.SUMMIN_GET)
        self._check_ec(li, e, ExceptionKey.SUMMIN_GET)
        return self._sum_min.get((s, li, e, d), self._sum_min_def[li, e])

    def set_sum_min(
        self, s: StageId, li: NetLinkId, e: EcId, d: NetLinkDirection, sum_min: Value
    ) -> None:
        """
        ...
        :param d: Direction (forward or backward)
        :type d: NetLinkDirection
        :param sum_min: Minimal energy that has to be transported throughout the stage
        :type sum_min: Value
        """
        self._check_id(li, ExceptionKey.SUMMIN_SET)
        self._check_ec(li, e, ExceptionKey.SUMMIN_SET)
        expected_unit = self._sum_min_def[li, e].unit
        if not sum_min.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.SUMMIN_SET.value,
                [s, li, e],
                f"Unit {sum_min.unit} of sum_min[{s}, {li}, {e}, {d}] "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    def get_sum_max(
        ...
        self._check_ec(li, e, ExceptionKey.SUMMAX_GET)
        return self._sum_max.get((s, li, e, d), self._sum_max_def[li, e])

    def set_sum_max(
        self, s: StageId, li: NetLinkId, e: EcId, d: NetLinkDirection, sum_max: Value
    ) -> None:
        """
        ...
        :param d: Direction (forward or backward)
        :type d: NetLinkDirection
        :param sum_max: Maximal energy that may be transported throughout the stage
        :type sum_max: Value
        """
        self._check_id(li, ExceptionKey.SUMMAX_SET)
        self._check_ec(li, e, ExceptionKey.SUMMAX_SET)
        expected_unit = self._sum_max_def[li, e].unit
        if not sum_max.unit.same_type_as(expected_unit):
            raise exceptions.DataException(
                ExceptionKey.SUMMAX_SET.value,
                [s, li, e],
                f"Unit {sum_max.unit} of sum_max[{s}, {li}, {e}, {d}] "
                f"does not match expected unit {expected_unit}",
                module=LOG_MODULE_STR,
            )
    ...
    def set_time_series_val(
        ...
        if kind == TimeSeriesKind.NETLINKAVAIL:
            ...
            e = EcId(ids[1])
            unit = DimlessUnit()
            self.set_availability(s, li, e, t, Value(value, unit=unit))
    ...
    def __init__(self) -> None:
        ...
        self._length: Dict[NetLinkId, Value] = {}
        self._bidirectional: Dict[NetLinkId, bool] = {}
        self._cap_min: Dict[Tuple[StageId, NetLinkId, EcId], Value] = {}
        self._cap_max: Dict[Tuple[StageId, NetLinkId, EcId], Value] = {}
        self._cap_min_def: Dict[Tuple[NetLinkId, EcId], Value] = {}
        self._cap_max_def: Dict[Tuple[NetLinkId, EcId], Value] = {}
        self._availability: Dict[Tuple[StageId, NetLinkId, EcId], TimeSeries] = {}
        self._sum_min: Dict[
            Tuple[StageId, NetLinkId, EcId, NetLinkDirection], Value
        ] = {}
        self._sum_min_def: Dict[Tuple[NetLinkId, EcId], Value] = {}
        self._sum_max: Dict[
            Tuple[StageId, NetLinkId, EcId, NetLinkDirection], Value
        ] = {}
        self._sum_max_def: Dict[Tuple[NetLinkId, EcId], Value] = {}
    ...
    def _check_ec(self, li: NetLinkId, e: EcId, where: ExceptionKey) -> None:
        if e not in self._ecs.get(li, set()):
            raise exceptions.DataException(
                where.value,
                [li, e],
                f"Encountered ec {e} which is not assigned to network link {li} "
                f"while {where.value}",
                module=LOG_MODULE_STR,
            )
