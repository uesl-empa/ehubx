...
def _build_base(
    ...
    load_shedding_tuples = load_shedding.get_enabled_tuples(stages, hubs, ecs, demands)
    setattr(
        model,
...
def _build_cost(model: Model, load_shedding: LoadShedding, times: Times) -> None:
    # [VAR] Load shedding cost
    setattr(
        model,
        VAR_LOADHSHEDDINGCOST,
        Var(getattr(model, SET_LOADSHEDDINGTUPLE), domain=NonNegativeReals),
    )
...
def _con_load_shedding_cost(
    model: Model, load_shedding: LoadShedding, times: Times
) -> None:
...
def _con_load_shedding_max(
    model: Model, demands: Demands, load_shedding: LoadShedding
) -> None:
