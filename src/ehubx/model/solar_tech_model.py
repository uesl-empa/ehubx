...
CON_SOLARTECHINMAXINCIDENTAVAILABILITY: str = \
    "C_SolarTechInMaxIncidentAvailability"
...
CON_SOLARTECHINMINCURTAILAVAILABILITY: str = \
    "C_SolarTechInMinCurtailAvailability"
...
def build(model: Model, conv_techs: ConversionTechs, solar_techs: SolarTechs,
    ...
    setattr(model, SET_SOLARTECHTUPLE,
            Set(within=getattr(model, SET_CONVTECHTUPLE),
                initialize=[(s, h, x)
                            for (s, h, x) in getattr(model, SET_CONVTECHTUPLE)
                            if TechId(x) in solar_techs.ids]))
    # [VAR] Solar incident
    setattr(model, VAR_SOLARTECHINCIDENT,
            Var(getattr(model, SET_SOLARTECHTUPLE), getattr(model, SET_TIME),
    ...
    setattr(model, SET_SOLAREC,
            Set(within=getattr(model, SET_EC), initialize=e_solars))
...
def _con_solar_tech_incident(model: Model, conv_techs: ConversionTechs,
                             solar_data: SolarData) -> None:

    def __rule_solarar_tech_incident(model, s, h, x, t):
        # Get parameters
        e_solar = conv_techs.get_in_ec_main(TechId(x))
        solar_irradiation = solar_data.get_irradiation(StageId(s), e_solar
                                                       ).get_value(TimeId(t))
        # Calculate solar incident
        solar_tech_incident = (solar_irradiation
                               * getattr(model, VAR_TECHCAP)[s, h, x])
        # Set constraint
        return (getattr(model, VAR_SOLARTECHINCIDENT)[s, h, x, t]
                == solar_tech_incident)

    setattr(model, CON_SOLARTECHINCIDENT,
            Constraint(getattr(model, SET_SOLARTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_solarar_tech_incident))


def _con_solar_tech_in_max_incident_availability(model: Model,
    ...
    def __rule_solarar_tech_in_max_incident_availability(model, s, h, x, t):
        ...
        # Set constraint
        return (getattr(model, VAR_CONVTECHIN)[s, h, x, e_solar.key, t]
                <= in_max)

    setattr(model, CON_SOLARTECHINMAXINCIDENTAVAILABILITY,
            Constraint(getattr(model, SET_SOLARTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_solarar_tech_in_max_incident_availability))


def _con_solar_tech_in_min_curtail_availability(model: Model,
        conv_techs: ConversionTechs, solar_techs: SolarTechs) -> None:

    def __rule_solarar_tech_in_min_curtail_availability(model, s, h, x, t):
        # Get parameters
        e_solar = conv_techs.get_in_ec_main(TechId(x))
        curtail_max_rel = solar_techs.get_curtail_max_rel(StageId(s),
                                                          TechId(x))
        availability = conv_techs.get_availability(StageId(s), HubId(h),
            TechId(x)).get_value(TimeId(t))
        # Calculate minimal input
        in_min = ((1 - curtail_max_rel)
                  * availability
                  * getattr(model, VAR_SOLARTECHINCIDENT)[s, h, x, t])
        # Set constraint
        return (getattr(model, VAR_CONVTECHIN)[s, h, x, e_solar.key, t]
    ...
    setattr(model, CON_SOLARTECHINMINCURTAILAVAILABILITY,
            Constraint(getattr(model, SET_SOLARTECHTUPLE),
                       getattr(model, SET_TIME),
...
def _con_deactivate_conv_tech_cap_and_availability(model: Model) -> None:
    for (s, h, x) in getattr(model, SET_SOLARTECHTUPLE):
        for t in getattr(model, SET_TIME):
            getattr(model, CON_CONVTECHCAPANDAVAILABILITY
                    )[s, h, x, t].deactivate()


def _con_solar_area(model: Model, conv_techs: ConversionTechs,
                    solar_data: SolarData) -> None:
    ...
    setattr(model, CON_SOLARAREA,
            Constraint(getattr(model, SET_STAGE), getattr(model, SET_HUB),
                       getattr(model, SET_SOLAREC), rule=__rule_solar_area))
