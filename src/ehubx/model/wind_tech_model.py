...
def build(
    ...
    # [SET] Windparks
    setattr(
        model, SET_WINDPARK, Set(initialize=[wp.key for wp in wind_data.windpark_ids])
    )
...
def _con_wind_tech_incident(
    ...
    def __rule_wind_tech_incident(model, s, h, x, t):
        ...
        turbine_footprint = wind_techs.get_turbine_footprint(StageId(s), TechId(x))
        ...
        if (velo >= velo_cut_in) and (velo <= velo_nominal):
            wind_tech_incident = (
                linear_factor * getattr(model, VAR_TECHCAP)[s, h, x] * velo**3
            )
        if (velo > velo_nominal) and (velo <= velo_cut_off):
            wind_tech_incident = (
                linear_factor * getattr(model, VAR_TECHCAP)[s, h, x] * velo_nominal**3
            )
        # Multiply incident by tech availability
        wind_tech_incident *= availability
        # Set the constraint
        return getattr(model, VAR_WINDTECHINCIDENT)[s, h, x, t] == wind_tech_incident

    setattr(
        model,
        CON_WINDTECHINCIDENT,
        Constraint(
            getattr(model, SET_WINDTECHTUPLE),
            getattr(model, SET_TIME),
...
def _con_wind_tech_curtail_max(model: Model, wind_techs: WindTechs) -> None:
    def __rule_wind_tech_curtail_max(model, s, h, x, t):
        ...
        curtail_max = curtail_max_rel * getattr(model, VAR_WINDTECHINCIDENT)[s, h, x, t]
        # Set the constraint
        return getattr(model, VAR_WINDTECHCURTAILMENT)[s, h, x, t] <= curtail_max

    setattr(
        model,
        CON_WINDTECHCURTAILMAX,
        Constraint(
            getattr(model, SET_WINDTECHTUPLE),
            getattr(model, SET_TIME),
...
def _con_wind_tech_in(model: Model, conv_techs: ConversionTechs) -> None:
    def __rule_wind_tech_in(model, s, h, x, t):
        ...
        # Set the constraint
        return getattr(model, VAR_CONVTECHIN)[s, h, x, e_wind.key, t] == wind_tech_in
...
def _con_wind_tech_area_min(model: Model, wind_techs: WindTechs) -> None:
    def __rule_wind_tech_area_min(model, s, h, x):
        ...
        min_area = turbine_footprint * getattr(model, VAR_YTECHCAPINSTL)[s, h, x]
        # Set the constraint
    ...
    setattr(
        model,
        CON_WINDTECHAREAMIN,
        Constraint(getattr(model, SET_WINDTECHTUPLE), rule=__rule_wind_tech_area_min),
    )


def _con_windpark_area(
    model: Model, conv_techs: ConversionTechs, wind_data: WindData
) -> None:
    def __rule_windpark_area(model, s, h, wp):
        # Get available windpark area
        windpark_area = wind_data.get_windpark_area(
            StageId(s), HubId(h), WindparkId(wp)
        )
        ...
        installed_area = sum(
            ...
            if h_ == h
            if (
                conv_techs.get_in_ec_main(TechId(x))
