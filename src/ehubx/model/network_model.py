...
def build(model: Model, stages: Stages, hubs: Hubs, net_links: NetworkLinks,
          net_techs: NetworkTechs, times: Times) -> None:
    """
    ...
    logging.log_file(
        "Built network module. Elapsed time: "
        f"{int(elapsed.total_seconds())}s", module=LOG_MODULE_STR)


def _build_base_trans(model: Model, hubs: Hubs, net_links: NetworkLinks,
                      net_techs: NetworkTechs, times: Times) -> None:
    # [SET] Network Links
    setattr(model, SET_NETLINK,
            Set(initialize=[li.key for li in net_links.ids]))
    # [SET] Network techs
    setattr(model, SET_NETTECH,
            Set(initialize=[n.key for n in net_techs.ids]))
    # [SET] Tuples of (hub, link, ec) for which a network input could occur
    net_link_in = [(net_links.get_hub_start(li), li, e)
                   for li in net_links.ids
                   for e in net_links.get_ecs(li)]
    net_link_in += [(net_links.get_hub_end(li), li, e)
                    for li in net_links.ids
                    for e in net_links.get_ecs(li)
                    if net_links.is_bidirectional(li)]
    setattr(model, SET_NETLINKIN,
            Set(within=(getattr(model, SET_HUB)
                        * getattr(model, SET_NETLINK)
                        * getattr(model, SET_EC)),
                initialize=[(h.key, li.key, e.key)
                            for (h, li, e) in net_link_in]))
    # [VAR] Network input for tuples (hub, link, ec), summed across net_techs
    setattr(model, VAR_NETLINKIN,
            Var(getattr(model, SET_STAGE),
                getattr(model, SET_NETLINKIN),
                getattr(model, SET_TIME),
                domain=NonNegativeReals))
    # [SET] Tuples of (hub, link, ec) for which a network output could occur
    net_link_out = [(net_links.get_hub_end(li), li, e)
                    for li in net_links.ids
                    for e in net_links.get_ecs(li)]
    net_link_out += [(net_links.get_hub_start(li), li, e)
                     for li in net_links.ids
                     for e in net_links.get_ecs(li)
                     if net_links.is_bidirectional(li)]
    setattr(model, SET_NETLINKOUT,
            Set(within=(getattr(model, SET_HUB)
                        * getattr(model, SET_NETLINK)
                        * getattr(model, SET_EC)),
                initialize=[(h.key, li.key, e.key)
                            for (h, li, e) in net_link_out]))
    # [VAR] Network output for tuples (hub, link, ec), summed across net_techs
    setattr(model, VAR_NETLINKOUT,
            Var(getattr(model, SET_STAGE),
    ...
    setattr(model, SET_NETHUBIN,
            Set(within=(getattr(model, SET_HUB) * getattr(model, SET_EC)),
        initialize=[(h.key, e.key) for (h, e) in net_hub_in]))
    # [VAR] Network input for tuples (hub, ec), summed across links
    setattr(model, VAR_NETHUBIN,
            Var(getattr(model, SET_STAGE),
    ...
    net_tech_tuple = [(s.key, li.key, n.key)
                      for n in net_techs.ids
                      for s in net_techs.get_allowed_stages(n)
                      for li in net_techs.get_allowed_net_links(n)]
    setattr(model, SET_NETTECHTUPLE,
            Set(initialize=net_tech_tuple,
                within=(getattr(model, SET_STAGE)
                        * getattr(model, SET_NETLINK)
                        * getattr(model, SET_NETTECH))))

    # [SET] Tuples of (stage, hub, link, net_tech) for which a network input
    #       could occur
    net_tech_in = [(s, h.key, li, n)
                   for (s, li, n) in net_tech_tuple
                   for h in hubs.ids
                   if (h, NetLinkId(li),
                       net_techs.get_ec(NetTechId(n))) in net_link_in]
    setattr(model, SET_NETTECHIN,
            Set(initialize=net_tech_in,
                within=(getattr(model, SET_STAGE)
                        * getattr(model, SET_HUB)
                        * getattr(model, SET_NETLINK)
                        * getattr(model, SET_NETTECH))))
    # [VAR] Network input for tuples (stage, hub, link, net_tech)
    setattr(model, VAR_NETTECHIN,
            Var(getattr(model, SET_NETTECHIN), getattr(model, SET_TIME),
                domain=NonNegativeReals))
    # [SET] Tuples of (stage, hub, link, net_tech) for which a network output
    #       could occur
    net_tech_out = [(s, h.key, li, n)
                   for (s, li, n) in net_tech_tuple
                   for h in hubs.ids
                   if (h, NetLinkId(li),
                       net_techs.get_ec(NetTechId(n))) in net_link_out]
    setattr(model, SET_NETTECHOUT,
            Set(initialize=net_tech_out,
                within=(getattr(model, SET_STAGE)
                        * getattr(model, SET_HUB)
                        * getattr(model, SET_NETLINK)
                        * getattr(model, SET_NETTECH))))
    # [VAR] Network output for tuples (stage, hub, link, net_tech)
    setattr(model, VAR_NETTECHOUT,
            Var(getattr(model, SET_NETTECHOUT), getattr(model, SET_TIME),
                domain=NonNegativeReals))
    ...
    setattr(model, VAR_YNETTECHUSED,
...
def _build_base_cap(model: Model, stages: Stages, net_links: NetworkLinks,
                    net_techs: NetworkTechs) -> None:
    # [VAR] Network tech capacity
    setattr(model, VAR_NETTECHCAP,
            Var(getattr(model, SET_NETTECHTUPLE), domain=NonNegativeReals))
    # [SET] Tuples of links and ECs
    setattr(model, SET_NETLINKANDEC,
            Set(within=getattr(model, SET_NETLINK) * getattr(model, SET_EC),
                initialize=[(li.key, e.key)
                            for li in net_links.ids
                            for e in net_links.get_ecs(li)]))
    ...
    setattr(model, VAR_NETTECHCAPINSTL,
            Var(getattr(model, SET_NETTECHTUPLE), domain=NonNegativeReals))
    ...
    setattr(model, VAR_YNETTECHCAPINSTL,
            Var(getattr(model, SET_NETTECHTUPLE), domain=Binary))
...
def _build_cost(model: Model, stages: Stages, net_links: NetworkLinks,
                net_techs: NetworkTechs, times: Times) -> None:
    # [VAR] CAPEX cost
    setattr(model, VAR_NETTECHCOSTCAPEX,
    ...
    setattr(model, VAR_NETTECHCOSTOPEXCAP,
            Var(getattr(model, SET_NETTECHTUPLE), domain=Reals))
    ...
    setattr(model, VAR_NETTECHCOSTOPEXTRANS,
            Var(getattr(model, SET_NETTECHTUPLE), domain=Reals))
...
def _build_co2(model: Model, stages: Stages, net_links: NetworkLinks,
               net_techs: NetworkTechs, times: Times) -> None:
    # [VAR] CO2 emissions from installation
    setattr(model, VAR_NETTECHCO2INSTL,
            Var(getattr(model, SET_NETTECHTUPLE),
                domain=NonNegativeReals))
    ...
    setattr(model, VAR_NETTECHCO2TRANS,
            Var(getattr(model, SET_NETTECHTUPLE), domain=NonNegativeReals))
    ...
    setattr(model, VAR_NETTECHCO2TOTAL,
...
def _con_net_hub_inout(model: Model) -> None:
    ...
    def __rule_net_hub_in(model, s, h, e, t):
        ...
        net_link_in_sum = sum(getattr(model, VAR_NETLINKIN)[s, h, li, e, t]
...
def _con_net_link_inout(model: Model, net_techs: NetworkTechs) -> None:
    ...
    def __rule_net_link_in(model, s, h, li, e, t):
        ...
        # Set the constraint
        return (getattr(model, VAR_NETLINKIN)[s, h, li, e, t]
                == net_tech_in_sum)
    ...
    def __rule_net_link_out(model, s, h, li, e, t):
        ...
        # Set the constraint
        return (getattr(model, VAR_NETLINKOUT)[s, h, li, e, t]
                == net_tech_out_sum)

    setattr(model, CON_NETLINKIN,
            Constraint(getattr(model, SET_STAGE),
                       getattr(model, SET_NETLINKIN),
                       getattr(model, SET_TIME),
                       rule=__rule_net_link_in))
    setattr(model, CON_NETLINKOUT,
            Constraint(getattr(model, SET_STAGE),
                       getattr(model, SET_NETLINKOUT),
                       getattr(model, SET_TIME),
...
def _con_net_trans_dynamic(model: Model, net_links: NetworkLinks,
                          net_techs: NetworkTechs) -> None:
    ...
    def __rule_net_trans_dynamic(model, s, h_out, li, n, t):
        ...
        net_tech_out = ((1 - trans_loss) ** link_length
                        * getattr(model, VAR_NETTECHIN)[s, h_in.key, li, n, t])
        # Set constraint
        return (getattr(model, VAR_NETTECHOUT)[s, h_out, li, n, t]
                == net_tech_out)

    setattr(model, CON_NETTRANSDYNAMIC,
            Constraint(getattr(model, SET_NETTECHOUT),
                       getattr(model, SET_TIME),
                       rule=__rule_net_trans_dynamic))


def _con_net_trans_sum_minmax(model: Model, net_links: NetworkLinks,
                              times: Times) -> None:
    ...
    def __rule_net_trans_sum_min(model, s, h_out, li, e):
        ...
        trans_sum_min = net_links.get_sum_min(StageId(s), NetLinkId(li),
                                              EcId(e), link_dir)
        # Calculate summed-up transmission
        trans_sum = sum((times.get_weight(StageId(s), TimeId(t))
    ...
    def __rule_net_trans_sum_max(model, s, h_out, li, e):
        ...
        if HubId(h_out) == net_links.get_hub_start(NetLinkId(li)):
            link_dir = NetLinkDirection.BACKWARD
        trans_sum_max = net_links.get_sum_max(StageId(s), NetLinkId(li),
                                              EcId(e), link_dir)
        # Calculate summed-up transmission
        trans_sum = sum((times.get_weight(StageId(s), TimeId(t))
                         * getattr(model, VAR_NETLINKOUT)[s, h_out, li, e, t])
    ...
    setattr(model, CON_NETTRANSSUMMIN,
            Constraint(getattr(model, SET_STAGE),
                       getattr(model, SET_NETLINKOUT),
                       rule=__rule_net_trans_sum_min))
    setattr(model, CON_NETTRANSSUMMAX,
            Constraint(getattr(model, SET_STAGE),
                       getattr(model, SET_NETLINKOUT),
                       rule=__rule_net_trans_sum_max))


def _con_net_tech_used(model: Model, net_links: NetworkLinks,
                       net_techs: NetworkTechs, times: Times) -> None:
    ...
    def __rule_net_tech_used(model, s, li, n):
        ...
        net_in_sum = sum((times.get_weight(StageId(s), TimeId(t))
                          * getattr(model, VAR_NETTECHIN)[s, h, li, n, t])
    ...
    setattr(model, CON_YNETTECHUSED,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_used))


def _con_net_tech_cap_minmax(model: Model, net_links: NetworkLinks,
                             net_techs: NetworkTechs) -> None:
    ...
    def __rule_net_tech_cap_min(model, s, li, e):
        ...
        cap_sum = sum(getattr(model, VAR_NETTECHCAP)[s, li, n]
    ...
    setattr(model, CON_NETTECHCAPMIN,
            Constraint(getattr(model, SET_STAGE),
                       getattr(model, SET_NETLINKANDEC),
                       rule=__rule_net_tech_cap_min))
    setattr(model, CON_NETTECHCAPMAX,
            Constraint(getattr(model, SET_STAGE),
                       getattr(model, SET_NETLINKANDEC),
                       rule=__rule_net_tech_cap_max))


def _con_net_tech_cap(model: Model, stages: Stages,
                      net_techs: NetworkTechs) -> None:
    ...
    setattr(model, CON_NETTECHCAP,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_cap))


def _con_net_tech_trans_cap_and_availability(model: Model,
    ...
    def __rule_net_tech_trans_cap_and_availability(model, s, li, n, t):
        ...
        availability = net_links.get_availability(StageId(s), NetLinkId(li),
                                                  e).get_value(TimeId(t))
        # Get inputs for this network tech from both hubs
        net_tech_in = sum(getattr(model, VAR_NETTECHIN)[s, h, li, n, t]
                          for h in getattr(model, SET_HUB)
                          if (s, h, li, n) in getattr(model, SET_NETTECHIN))
        # Set constraint
        return (net_tech_in
                <= availability * getattr(model, VAR_NETTECHCAP)[s, li, n])

    setattr(model, CON_NETTECHTRANSCAPANDAVAILABILITY,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_net_tech_trans_cap_and_availability))


def _con_y_net_tech_instl(model: Model, net_links: NetworkLinks,
                          net_techs: NetworkTechs) -> None:
    ...
    setattr(model, CON_YNETTECHINSTL,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_y_net_tech_instl))
...
def _con_net_tech_unit_cap_min(model: Model, net_techs: NetworkTechs) -> None:
    ...
    def __rule_net_tech_unit_cap_min(model, s, li, n):
        ...
        return (getattr(model, VAR_NETTECHCAPINSTL)[s, li, n]
                >= (unit_cap_min
                    * getattr(model, VAR_YNETTECHCAPINSTL)[s, li, n]))

    setattr(model, CON_NETTECHUNITCAPMIN,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_unit_cap_min))


def _con_net_tech_cost_capex(model: Model, stages: Stages,
                             net_links: NetworkLinks,
                             net_techs: NetworkTechs) -> None:
    ...
    def __rule_net_tech_cost_capex(model, s, li, n):
        ...
        for s_instl in getattr(model, SET_STAGE):
            ...
            capex_per_cap = net_techs.get_capex_per_cap(StageId(s_instl),
                                                        NetTechId(n))
            one_time_capex = net_techs.get_one_time_capex(StageId(s_instl),
                                                          NetTechId(n))
            # One-time capex costs (if installation occured)
            cost_capex += (crf * one_time_capex
                           * getattr(model,
                                     VAR_YNETTECHCAPINSTL)[s_instl, li, n])
            # Per-capacity capex costs
            cost_capex += (crf * capex_per_cap
                           * getattr(model,
                                     VAR_NETTECHCAPINSTL)[s_instl, li, n])
        # Multiply cost with link length
        cost_capex *= link_length
        # Set constraint
        return getattr(model, VAR_NETTECHCOSTCAPEX)[s, li, n] == cost_capex

    setattr(model, CON_NETTECHCOSTCAPEX,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_cost_capex))


def _con_net_tech_cost_opex_cap(model: Model, net_links: NetworkLinks,
                                net_techs: NetworkTechs) -> None:
    ...
    def __rule_net_tech_cost_opex_cap(model, s, li, n):
        ...
        # Calculate OPEX cost
        cost_opex_instl = (one_time_opex * getattr(model,
                                                   VAR_YNETTECHUSED)[s, li, n]
                           + opex_per_cap * getattr(model,
                                                    VAR_NETTECHCAP)[s, li, n])
        # Multiply cost with link length
        cost_opex_instl *= link_length
        # Set constraint
        return (getattr(model, VAR_NETTECHCOSTOPEXCAP)[s, li, n]
                == cost_opex_instl)

    setattr(model, CON_NETTECHCOSTOPEXCAP,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_cost_opex_cap))


def _con_net_tech_cost_opex_trans(model: Model, net_links: NetworkLinks,
                                  net_techs: NetworkTechs, times: Times
                                  ) -> None:

    def __rule_net_tech_cost_opex_trans(model, s, li, n):
        # Parameters
        link_length = net_links.get_length(NetLinkId(li))
        opex_per_energy = net_techs.get_opex_per_energy(StageId(s),
                                                        NetTechId(n))
        # Get summed-up input for this link from both hubs
        net_in_sum = sum(times.get_weight(StageId(s), TimeId(t))
                         * getattr(model, VAR_NETTECHIN)[s, h, li, n, t]
        ...
        # Set constraint
        return (getattr(model, VAR_NETTECHCOSTOPEXTRANS)[s, li, n]
                == cost_opex_trans)

    setattr(model, CON_NETTECHCOSTOPEXTRANS,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_cost_opex_trans))
...
def _con_net_tech_cost_total(model: Model) -> None:
    ...
    setattr(model, CON_NETTECHCOSTTOTAL,
            Constraint(rule=__rule_net_tech_cost_total))


def _con_net_tech_co2_instl(model: Model, stages: Stages,
                            net_links: NetworkLinks, net_techs: NetworkTechs
                            ) -> None:
    ...
    setattr(model, CON_NETTECHCO2INSTL,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_co2_instl))


def _con_net_tech_co2_trans(model: Model, net_links: NetworkLinks,
                            net_techs: NetworkTechs, times: Times) -> None:
    ...
    setattr(model, CON_NETTECHCO2TRANS,
            Constraint(getattr(model, SET_NETTECHTUPLE),
                       rule=__rule_net_tech_co2_trans))
...
def _con_net_tech_co2_total(model: Model) -> None:
    ...
    def __rule_net_tech_co2_total(model, s):
        ...
        net_tech_co2_total = sum(
            ...
            for (s_, li, n) in getattr(model, SET_NETTECHTUPLE) if s == s_)
        # Set the constraint
        return getattr(model, VAR_NETTECHCO2TOTAL)[s] == net_tech_co2_total

    setattr(model, CON_NETTECHCO2TOTAL,
            Constraint(getattr(model, SET_STAGE),
                       rule=__rule_net_tech_co2_total))
