...
def _build_modules(model: Model, energy_system: EnergySystem) -> None:
    ...
    demand_model.build(
        model, energy_system.stages, energy_system.demands, energy_system.times
    )
    load_shedding_model.build(
        model,
        energy_system.stages,
        energy_system.hubs,
        energy_system.ecs,
        energy_system.demands,
        energy_system.load_shedding,
        energy_system.times,
    )
    load_shifting_model.build(
        model, energy_system.demands, energy_system.load_shifting, energy_system.times
    )
    network_model.build(
        model,
        energy_system.stages,
        energy_system.hubs,
        energy_system.net_links,
        energy_system.net_techs,
        energy_system.times,
    )
    tech_model.build(model, energy_system.stages, energy_system.techs)
    stor_tech_model.build(
        model,
        energy_system.stages,
        energy_system.techs,
        energy_system.stor_techs,
        energy_system.times,
    )
    ebm_tech_model.build(
        model, energy_system.stages, energy_system.ebm_techs, energy_system.times
    )
    conv_tech_model.build(
        model, energy_system.techs, energy_system.conv_techs, energy_system.times
    )
    solar_tech_model.build(
        model,
        energy_system.conv_techs,
        energy_system.solar_techs,
        energy_system.solar_data,
    )
    wind_tech_model.build(
        model,
        energy_system.conv_techs,
        energy_system.wind_techs,
        energy_system.wind_data,
    )
    hp_tech_model.build(
        model, energy_system.techs, energy_system.hp_techs, energy_system.times
    )
    ates_tech_model.build(
        model,
        energy_system.stages,
        energy_system.hubs,
        energy_system.techs,
        energy_system.ates_techs,
        energy_system.ates_data,
        energy_system.times,
    )
    autarky_model.build(
        model,
        energy_system.stages,
        energy_system.ecs,
        energy_system.imports,
        energy_system.demands,
        energy_system.conv_techs,
        energy_system.autarky,
        energy_system.times,
    )
...
def _build_self_demand_supply(model: Model, demands: Demands) -> None:
    ...
    setattr(
        ...
        VAR_DEMANDSUPPLY,
        Var(
            getattr(model, demand_model.SET_DEMANDTUPLE),
...
def _con_system_cost_co2_penalty(model: Model, stages: Stages) -> None:
    def __rule_system_cost_co2_penalty(model):
        ...
        system_cost_co2_penalty = sum(
            (stages.get_co2_price(StageId(s)) * getattr(model, VAR_SYSTEMCO2)[s])
...
def _con_system_cost(model: Model) -> None:
    def __rule_system_cost_total_var(model):
        ...
        # Load shifting cost
        system_cost_total += getattr(
            model, load_shifting_model.VAR_LOADHSHIFTINGCOSTTOTAL
        )
...
def _con_system_autarky(model: Model) -> None:
    def __rule_system_autarky(model):
        return getattr(model, VAR_SYSTEMAUTARKY) == getattr(
            model, autarky_model.VAR_AUTARKY
        )

    setattr(model, CON_SYSTEMAUTARKY, Constraint(rule=__rule_system_autarky))
...
def _obj_system_autarky(model: Model):
    ...
    setattr(model, OBJ_SYSTEMAUTARKY, Objective(rule=__rule_system_autarky_obj))
...
def _con_demand_supply(model: Model, demands: Demands) -> None:
    def __rule_demand(model, s, h, e, t):
        demand_supply = demands.get_demand(StageId(s), HubId(h), EcId(e)).get_value(
            TimeId(t)
        )
        ...
        if (s, h, e) in getattr(model, load_shifting_model.SET_LOADSHIFTINGTUPLE):
            demand_supply += getattr(model, load_shifting_model.VAR_LOADSHIFTING)[
                s, h, e, t
            ]
