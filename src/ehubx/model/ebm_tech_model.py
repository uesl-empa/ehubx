...
def build(model: Model, stages: Stages, ebm_techs: EbmTechs, times: Times) -> None:
    """
...
def _build_base(
    model: Model, stages: Stages, ebm_techs: EbmTechs, times: Times
) -> None:
    # [SET] EBM techs
    setattr(
        model,
...
def _con_ebm_tech_inoutflow_max(model: Model, ebm_techs: EbmTechs) -> None:
    ...
    def __rule_ebm_tech_outflow_max(model, s, h, x, t):
        ...
        num_vehicles = ebm_techs.get_num_vehicles(StageId(s), HubId(h), TechId(x))
        availability_t = availability[s, h, x].get_value(TimeId(t))
        discharge_control = ebm_techs.get_discharge_control(StageId(s), TechId(x))
...
def _con_ebm_tech_used(model: Model, ebm_techs: EbmTechs, times: Times) -> None:
...
def _con_ebm_tech_charging_dynamic(
    model: Model, ebm_techs: EbmTechs, times: Times
) -> None:
...
def _con_ebm_tech_energy_init(
    model: Model, stages: Stages, ebm_techs: EbmTechs
) -> None:
