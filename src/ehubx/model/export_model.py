...
def build(model: Model, exports: Exports, times: Times) -> None:
    ...
    logging.log_file(
        f"Built export module. Elapsed time: {int(elapsed.total_seconds())}s",
...
def _build_base(model: Model, exports: Exports, times: Times) -> None:
    ...
    setattr(
        ...
        Set(
            ...
            initialize=[(s.key, h.key, e.key) for (s, h, e) in exports.tuples],
        ),
    )
    # [VAR] Export
    setattr(
        ...
        VAR_EXP,
        Var(
            (getattr(model, SET_EXPTUPLE) * getattr(model, SET_TIME)),
...
def _build_profit(model: Model, exports: Exports, times: Times) -> None:
    # [VAR] Export profit
    setattr(
        model, VAR_EXPPROFIT, Var(getattr(model, SET_EXPTUPLE), domain=NonNegativeReals)
    )
    # [CON] Export profit
    _con_exp_profit(model, exports, times)
    # [VAR] Total export profit
    setattr(model, VAR_EXPPROFITTOTAL, Var(domain=NonNegativeReals))
...
def _con_exp_minmax(model: Model, exports: Exports) -> None:
    def __rule_exp_min(model, s, h, e, t):
        exp_min = exports.get_min(StageId(s), HubId(h), EcId(e)).get_value(TimeId(t))
        return getattr(model, VAR_EXP)[s, h, e, t] >= exp_min

    def __rule_exp_max(model, s, h, e, t):
        exp_max = exports.get_max(StageId(s), HubId(h), EcId(e)).get_value(TimeId(t))
    ...
    setattr(
        ...
        CON_EXPMIN,
        Constraint(
            getattr(model, SET_EXPTUPLE), getattr(model, SET_TIME), rule=__rule_exp_min
        ),
    )
    setattr(
        model,
        CON_EXPMAX,
        Constraint(
            getattr(model, SET_EXPTUPLE), getattr(model, SET_TIME), rule=__rule_exp_max
        ),
    )
...
def _con_exp_profit_total(model: Model) -> None:
    ...
    setattr(model, CON_EXPPROFITTOTAL, Constraint(rule=__rule_exp_profit_total))
...
def _con_exp_co2(model: Model, exports: Exports, times: Times) -> None:
    ...
    setattr(
        model, CON_EXPCO2, Constraint(getattr(model, SET_EXPTUPLE), rule=__rule_exp_co2)
    )
