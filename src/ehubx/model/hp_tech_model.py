...
def build(model: Model, techs: Techs, hp_techs: HeatpumpTechs,
          times: Times) -> None:
    """
    ...
    logging.log_file(
        "Built heat pump tech module. Elapsed time: "
        f"{int(elapsed.total_seconds())}s", module=LOG_MODULE_STR)


def _build_base(model: Model, techs: Techs, hp_techs: HeatpumpTechs,
                times: Times) -> None:
    # [SET] Heat pump tech tuples
    setattr(model, SET_HPTECHTUPLE,
    ...
    setattr(model, SET_HPTECHIN,
            Set(within=(getattr(model, SET_HPTECHTUPLE)
                        * getattr(model, SET_EC)),
                initialize=[(s, h, x, e.key)
                            for (s, h, x) in getattr(model, SET_HPTECHTUPLE)
                            for e in [hp_techs.get_ec_el(TechId(x)),
                                      hp_techs.get_ec_ht_in(TechId(x)),
                                      hp_techs.get_ec_co_in(TechId(x))]]))
    # [SET] Tuples of heat pump tech tuples and their output ecs
    setattr(model, SET_HPTECHOUT,
            Set(within=(getattr(model, SET_HPTECHTUPLE)
                        * getattr(model, SET_EC)),
                initialize=[(s, h, x, e.key)
                            for (s, h, x) in getattr(model, SET_HPTECHTUPLE)
                            for e in [hp_techs.get_ec_ht_out(TechId(x)),
                                      hp_techs.get_ec_co_out(TechId(x))]]))
    # [VAR] Heat pump input
    setattr(model, VAR_HPTECHIN,
            Var(getattr(model, SET_HPTECHIN), getattr(model, SET_TIME),
                domain=NonNegativeReals))
    # [VAR] Heat pump output
    setattr(model, VAR_HPTECHOUT,
            Var(getattr(model, SET_HPTECHOUT), getattr(model, SET_TIME),
                domain=NonNegativeReals))
    # [VAR] Separate electricity consumption variables for heating/cooling
    setattr(model, VAR_HPTECHELECHT,
            Var(getattr(model, SET_HPTECHTUPLE) * getattr(model, SET_TIME),
                domain=NonNegativeReals))
    setattr(model, VAR_HPTECHELECCO,
            Var(getattr(model, SET_HPTECHTUPLE) * getattr(model, SET_TIME),
                domain=NonNegativeReals))
...
def _con_hp_tech_total_elec(model: Model, hp_techs: HeatpumpTechs) -> None:
    ...
    def __rule_hp_tech_total_elec(model, s, h, x, t):
        ...
        total_elec = (getattr(model, VAR_HPTECHELECHT)[s, h, x, t]
                      + getattr(model, VAR_HPTECHELECCO)[s, h, x, t])
        # Set constraint
        return (getattr(model, VAR_HPTECHIN)[s, h, x, ec_el.key, t]
                == total_elec)

    setattr(model, CON_HPTECHTOTALELEC,
            Constraint(getattr(model, SET_HPTECHTUPLE),
...
def _con_hp_tech_pow_bal_ht(model: Model, hp_techs: HeatpumpTechs) -> None:
    ...
    setattr(model, CON_HPTECHPOWBALHT,
            Constraint(getattr(model, SET_HPTECHTUPLE),
                       getattr(model, SET_TIME),
...
def _con_hp_tech_pow_bal_co(model: Model, hp_techs: HeatpumpTechs) -> None:
    ...
    setattr(model, CON_HPTECHPOWBALCO,
            Constraint(getattr(model, SET_HPTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_hp_tech_pow_bal_co))


def _con_hp_tech_cop(model: Model, hp_techs: HeatpumpTechs,
                     times: Times) -> None:

    # Precompute COP for efficiency
    cops = {(s, h, x): hp_techs.get_cop(StageId(s), HubId(h), TechId(x),
                                        times)
            for (s, h, x) in getattr(model, SET_HPTECHTUPLE)}
    ...
    setattr(model, CON_HPTECHCOPHT,
            Constraint(getattr(model, SET_HPTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_hp_tech_cop_ht))
    setattr(model, CON_HPTECHCOPCO,
            Constraint(getattr(model, SET_HPTECHTUPLE),
                       getattr(model, SET_TIME),
...
def _con_hp_tech_cap(model: Model, hp_techs: HeatpumpTechs) -> None:
    ...
    def __rule_hp_tech_cap(model, s, h, x, t):
        ...
        pow_cond = (getattr(model,
                            VAR_HPTECHOUT)[s, h, x, ec_ht_out.key, t]
                    + getattr(model, VAR_HPTECHIN)[s, h, x, ec_co_in.key, t])
        # Set constraint
        return pow_cond <= getattr(model, VAR_TECHCAP)[s, h, x]

    setattr(model, CON_HPTECHCAP,
            Constraint(getattr(model, SET_HPTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_hp_tech_cap))


def _con_hp_tech_used(model: Model, techs: Techs,
                      hp_techs: HeatpumpTechs, times: Times) -> None:
    ...
    setattr(model, CON_HPTECHUSED,
            Constraint(getattr(model, SET_HPTECHTUPLE),
                       rule=__rule_hp_tech_used))
