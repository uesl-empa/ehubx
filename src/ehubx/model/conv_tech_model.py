...
def build(model: Model, techs: Techs, conv_techs: ConversionTechs,
          times: Times) -> None:
    """
    ...
    logging.log_file(
        "Built conversion tech module. Elapsed time: "
        f"{int(elapsed.total_seconds())}s", module=LOG_MODULE_STR)


def _build_base(model: Model, techs: Techs, conv_techs: ConversionTechs,
                times: Times) -> None:
    # [SET] Conversion tech tuples
    setattr(model, SET_CONVTECHTUPLE,
    ...
    setattr(model, VAR_CONVTECHIN,
            Var(getattr(model, SET_CONVTECHIN), getattr(model, SET_TIME),
                domain=NonNegativeReals))
    # [VAR] Conversion output
    setattr(model, VAR_CONVTECHOUT,
            Var(getattr(model, SET_CONVTECHOUT), getattr(model, SET_TIME),
                domain=NonNegativeReals))
...
def _build_cost(model: Model, conv_techs: ConversionTechs, times: Times
                ) -> None:
    # [VAR] OPEX (operation & maintenance) cost from conversion output
    setattr(model, VAR_CONVTECHCOSTOPEXOUT,
    ...
    # [VAR] Total cost
    setattr(model, VAR_CONVTECHCOSTTOTAL,
...
def _con_conv_tech_in_part(model: Model, conv_techs: ConversionTechs) -> None:
    ...
    def __rule_conv_tech_in_part(model, s, h, x, e, t):
        ...
        in_total = sum(getattr(model, VAR_CONVTECHIN)[s, h, x, e_.key, t]
                       for e_ in conv_techs.get_in_ecs(TechId(x)))
        # Calculate total in_parts
        in_part_total = sum(conv_techs.get_in_part(StageId(s),
                                                   TechId(x), e_)
                            for e_ in conv_techs.get_in_ecs(TechId(x)))
        # Set constraint
    ...
    setattr(model, CON_CONVTECHINPART,
            Constraint(getattr(model, SET_CONVTECHIN),
...
def _con_conv_tech_out_eff(model: Model, conv_techs: ConversionTechs) -> None:
    ...
    def __rule_conv_tech_out_eff(model, s, h, x, e, t):
        ...
        efficiency = conv_techs.get_out_eff(StageId(s), TechId(x), EcId(e)
                                            ).get_value(TimeId(t))
        # Calculate conversion output based on efficiency and main ec input:
        out = (efficiency
               * getattr(model, VAR_CONVTECHIN)[s, h, x, in_ec_main.key, t])
        # Set constraint
        return getattr(model, VAR_CONVTECHOUT)[s, h, x, e, t] == out

    setattr(model, CON_CONVTECHOUTEFF,
            Constraint(getattr(model, SET_CONVTECHOUT),
                       getattr(model, SET_TIME),
                       rule=__rule_conv_tech_out_eff))


def _con_conv_tech_used(model: Model, techs: Techs,
                        conv_techs: ConversionTechs, times: Times) -> None:
    ...
    def __rule_conv_tech_used(model, s, h, x):
        ...
        out_sum = sum(times.get_weight(StageId(s), TimeId(t))
                      * getattr(model, VAR_CONVTECHOUT)[
                          s, h, x, out_ec_main.key, t]
                      for t in getattr(model, SET_TIME))
        # Set constraint
        return out_sum <= big_m * getattr(model, VAR_YTECHUSED)[s, h, x]

    setattr(model, CON_CONVTECHUSED,
            Constraint(getattr(model, SET_CONVTECHTUPLE),
                       rule=__rule_conv_tech_used))


def _con_conv_tech_cap_and_availability(model: Model,
    ...
    def __rule_conv_tech_cap_and_availability(model, s, h, x, t):
        ...
        # Set constraint
        return (getattr(model, VAR_CONVTECHOUT)[s, h, x, out_ec_main.key, t]
                <= out_max)

    setattr(model, CON_CONVTECHCAPANDAVAILABILITY,
            Constraint(getattr(model, SET_CONVTECHTUPLE),
                       getattr(model, SET_TIME),
                       rule=__rule_conv_tech_cap_and_availability))


def _con_conv_tech_out_sum_minmax(model: Model, conv_techs: ConversionTechs,
                                  times: Times) -> None:

    def __rule_conv_tech_out_sum_min(model, s, h, x):
        # Get parameters
        out_ec_main = conv_techs.get_out_ec_main(TechId(x))
        out_sum_min = conv_techs.get_out_sum_min(StageId(s), HubId(h),
                                                 TechId(x))
    ...
    setattr(model, CON_CONVTECHOUTSUMMIN,
            Constraint(getattr(model, SET_CONVTECHTUPLE),
                       rule=__rule_conv_tech_out_sum_min))
    setattr(model, CON_CONVTECHOUTSUMMAX,
            Constraint(getattr(model, SET_CONVTECHTUPLE),
                       rule=__rule_conv_tech_out_sum_max))


def _con_conv_tech_cost_opex_out(model: Model, conv_techs: ConversionTechs,
                                 times: Times) -> None:
    ...
    def __rule_conv_tech_cost_opex_out(model, s, h, x):
        ...
        # Set constraint
        return (getattr(model, VAR_CONVTECHCOSTOPEXOUT)[s, h, x]
                == cost_opex_out)

    setattr(model, CON_CONVTECHCOSTOPEXOUT,
            Constraint(getattr(model, SET_CONVTECHTUPLE),
                       rule=__rule_conv_tech_cost_opex_out))
...
def _con_conv_tech_cost_total(model: Model) -> None:
    ...
    setattr(model, CON_CONVTECHCOSTTOTAL,
            Constraint(rule=__rule_conv_tech_cost_total))
