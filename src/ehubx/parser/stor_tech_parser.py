...
def parse_primary(
    ...
    for tech_node in techs_node:
        _parse_stor_tech_primary(tech_node, stages, ecs, techs, stor_techs)
    # Logging
    _log(stor_techs)
    # Return
    return stor_techs
...
def _parse_stor_tech_primary(
    ...
    yaml_parser.check_node_type(storage_params_node, yaml_parser.YamlNodeKind.DICT)
    # ec
    ...
    in_eff = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        storage_params_node, YAMLKEY_INEFF, stages, expected_unit=DimlessUnit()
    )
    ...
    out_eff = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        storage_params_node, YAMLKEY_OUTEFF, stages, expected_unit=DimlessUnit()
    )
    ...
    soc_min = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        storage_params_node, YAMLKEY_SOCMIN, stages, expected_unit=DimlessUnit()
    )
    ...
    soc_max = yaml_parser.parse_optional_yeardep_value_from_dict_node(
        storage_params_node, YAMLKEY_SOCMAX, stages, expected_unit=DimlessUnit()
    )
...
def parse_secondary(
    hub_root_node: Optional[yaml_parser.YamlNode], stor_techs: StorageTechs
) -> None:
...
def _parse_hub_secondary(
    hub_node: yaml_parser.YamlDictNode, stor_techs: StorageTechs
) -> None:
    # id
...
def _parse_tech_secondary(
    ...
    soc_init = yaml_parser.parse_optional_value_from_dict_node(
        storage_params_node, YAMLKEY_SOCINIT, expected_unit=DimlessUnit()
    )
...
def _log(stor_techs: StorageTechs) -> None:
    logging.log_file(
        f"Parsed {len(stor_techs.ids)} storage tech(s)", module=LOG_MODULE_STR
    )
    for x in stor_techs.ids:
        logging.log_file(f"  StorTech {x}: ec {stor_techs.get_ec(x)}", print_time=False)
