...
def parse(demand_root_node: Optional[yaml_parser.YamlNode]) -> LoadShedding:
    ...
    if load_shedding_node is None:
        return load_shedding
    yaml_parser.check_node_type(load_shedding_node,
...
def _parse_preset(preset_node: yaml_parser.YamlDictNode,
...
def _parse_manual(manual_node: yaml_parser.YamlListNode,
                  load_shedding: LoadShedding) -> None:
    ...
    for node in manual_node:
        ...
        if len(dupe_tuples) > 0:
            msg = "Overlap detected in load shedding module: " + \
                f"The (stage, hub, ec) tuples {dupe_tuples} occur in more " + \
                "than one manual load shedding entry"
            raise exceptions.ParsingException(manual_node.file_path, msg,
...
def _parse_manual_defaults(node: yaml_parser.YamlDictNode,
    ...
    for (stage_id, hub_id, ec_id) in id_tuples:
        ...
        if energy_cost is not None:
            load_shedding.set_energy_cost_def(stage_id, hub_id, ec_id,
                                              energy_cost)


def _parse_manual_profiles(node: yaml_parser.YamlDictNode,
                           id_tuples: Set[Tuple[StageId, HubId, EcId]],
                           load_shedding: LoadShedding) -> None:
    # profile_path
    ...
    for (s, h, e, profile_key) in df_profiles.columns:
        ...
        if profile_key == YAMLKEY_MAXABS:
            for t in df_profiles.index:
                load_shedding.set_max_abs(stage_id, hub_id, ec_id, TimeId(t),
                                          df_profiles[s, h, e, profile_key][t])
        # max_rel
        if profile_key == YAMLKEY_MAXREL:
            for t in df_profiles.index:
                load_shedding.set_max_rel(stage_id, hub_id, ec_id, TimeId(t),
                                          df_profiles[s, h, e, profile_key][t])
        # energy_cost
        if profile_key == YAMLKEY_ENERGYCOST:
            for t in df_profiles.index:
                load_shedding.set_energy_cost(stage_id, hub_id, ec_id,
                    TimeId(t), df_profiles[s, h, e, profile_key][t])


def _log(load_shedding: LoadShedding) -> None:
    msg = "Load shedding "
    if load_shedding.enabled_preset:
        msg += (f"enabled as preset (max_abs={load_shedding.max_abs_preset}, "
    ...
    for (s, h, e) in load_shedding.manual_tuples:
        msg = (f"  Load shedding tuple ({s}, {h}, {e}): "
               + ("Enabled" if load_shedding.is_enabled(s, h, e)
                  else "Disabled"))
