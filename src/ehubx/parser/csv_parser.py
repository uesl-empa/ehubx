...
def parse(file_path: str,
          header_ids: Optional[List[HeaderId]] = None) -> pd.DataFrame:
    if not os.path.isfile(file_path):
        raise exceptions.MissingFileException(file_path, FILETYPE_CSV,
...
def _check_header(file_path: str, header_ids_as_str: List[str]) -> None:
    while True:
        try:
            df_preview = pd.read_csv(file_path, nrows=len(header_ids_as_str),
                                     header=None)
            break
        except PermissionError:
            logging.pause_console_log(write_console_entry=False)
            ...
            raise exceptions.ParsingException(file_path, "File is empty",
                                              module=LOG_MODULE_STR) from exc
    header_ids_in_csv = {df_preview.loc[row_pos, 0]
                         for row_pos in range(len(header_ids_as_str))}
    missing_header_ids = set(header_ids_as_str).difference(header_ids_in_csv)
    if len(missing_header_ids) > 0:
        msg = "csv_parser could not find the expected header ids " + \
            f"{missing_header_ids} (located in the starting rows of the " + \
            "first column)"
        raise exceptions.ParsingException(file_path, msg,
                                          module=LOG_MODULE_STR)
    # Check for header duplicates
    headers = [tuple(df_preview[col_no])
               for col_no in range(1, len(df_preview.columns + 1))]
    header_dupes = [header
                    for header, cnt in collections.Counter(headers).items()
                    if cnt > 1]
    if len(header_dupes) > 0:
        msg = "csv_parser encountered duplicate headers " + \
            f"{set(header_dupes)} (located in the starting rows)"
        raise exceptions.ParsingException(file_path, msg,
                                          module=LOG_MODULE_STR)


def _format(df: pd.DataFrame, header_ids_as_str: List[str]) -> pd.DataFrame:
