...
def _parse_tech(
    tech_node: yaml_parser.YamlDictNode, stages: Stages, wind_techs: WindTechs
) -> None:
    # tech_id
    ...
    # turbine_footprint
    turbine_footprint = yaml_parser.parse_mandatory_yeardep_float_from_dict_node(
...
def parse_data(
    renewables_subpath: str, ec_root_node: Optional[yaml_parser.YamlNode]
) -> WindData:
    wind_data = WindData()
    ...
    if os.path.isfile(areas_file_path):
        ...
        for s, h in df_area.columns:
            for wp in df_area.index:
                area = df_area[(s, h)][wp]
                wind_data.set_windpark_area(StageId(s), HubId(h), WindparkId(wp), area)
    # wind_velocity
    velocity_file_path = os.path.join(renewables_subpath, FILENAME_WINDVELOCITY)
    if os.path.isfile(velocity_file_path):
        df_velocity = csv_parser.parse(
            velocity_file_path,
        ...
        for s, e in df_velocity.columns:
            for t in df_velocity.index:
                ...
                wind_data.set_velocity(StageId(s), EcId(e), TimeId(t), velocity)
    # Logging
...
def _parse_windparks(
    ec_root_node: Optional[yaml_parser.YamlNode], wind_data: WindData
) -> None:
...
def _parse_windpark(
    windpark_node: yaml_parser.YamlDictNode, wind_data: WindData
) -> None:
    # windpark_id
...
def _log_wind_data(wind_data: WindData) -> None:
    logging.log_file(
        (
            f"Parsed wind data: Wind ecs = {wind_data.ecs}, "
    ...
    for wp in wind_data.windpark_ids:
        logging.log_file(
            f"  Windpark {wp}: ecs {wind_data.get_windpark_ecs(wp)}", print_time=False
        )
