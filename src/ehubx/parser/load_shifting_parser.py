...
def parse(demand_root_node: Optional[yaml_parser.YamlNode]) -> LoadShifting:
    ...
    for load_shift_node in load_shifts_node:
        yaml_parser.check_node_type(load_shift_node, yaml_parser.YamlNodeKind.DICT)
...
def _parse_load_shift(
    load_shift_node: yaml_parser.YamlDictNode, load_shifting: LoadShifting
) -> None:
    # load_shift_id
...
def _parse_defaults(
    load_shift_node: yaml_parser.YamlDictNode,
    load_shift_id: LoadShiftId,
    load_shifting: LoadShifting,
) -> None:
    # stages, hubs, ecs
    stages_str = yaml_parser.parse_str_list_from_dict_node(
        load_shift_node, YAMLKEY_STAGES
    )
    hubs_str = yaml_parser.parse_str_list_from_dict_node(load_shift_node, YAMLKEY_HUBS)
    ecs_str = yaml_parser.parse_str_list_from_dict_node(load_shift_node, YAMLKEY_ECS)
    ...
    for stage_id, hub_id, ec_id in itertools.product(stages, hubs, ecs):
        for other_load_shift_id in load_shifting.ids:
            if other_load_shift_id == load_shift_id:
                continue
            if (stage_id, hub_id, ec_id) in load_shifting.get_tuples(
                other_load_shift_id
            ):
                msg = (
                    "Duplicate (stage, hub, ec) tuple detected for the "
                ...
                raise exceptions.ParsingException(
                    load_shift_node.file_path, msg, module=LOG_MODULE_STR
                )
...
def _parse_profiles(
    ...
    profile_path = os.path.abspath(
        os.path.join(load_shift_node.file_path, os.pardir, profile_path)
    )
    yaml_parser.check_file_exists(profile_path, FILETYPE_LOADSHIFTPROFILE)
    df_profiles = csv_parser.parse(
        profile_path,
        header_ids=[csv_parser.HeaderId.LOADSHIFTID, csv_parser.HeaderId.PROFILEKEY],
    )
        ...
        if profile_key == YAMLKEY_MAXABOVEABS:
            for t in df_profiles.index:
                load_shifting.set_max_above_abs(
                    load_shift_id, TimeId(t), df_profiles[ls, profile_key][t]
                )
        # max_below_abs
