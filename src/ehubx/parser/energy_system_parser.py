...
from ehubx.parser import stage_parser
from ehubx.parser import hub_parser
from ehubx.parser import net_link_parser
from ehubx.parser import tech_parser
from ehubx.parser import conv_tech_parser
from ehubx.parser import solar_parser
from ehubx.parser import wind_parser
from ehubx.parser import hp_tech_parser
from ehubx.parser import stor_tech_parser
from ehubx.parser import ebm_tech_parser
from ehubx.parser import net_tech_parser
from ehubx.parser import ec_parser
from ehubx.parser import import_export_parser
from ehubx.parser import demand_parser
from ehubx.parser import load_shedding_parser
from ehubx.parser import load_shifting_parser
from ehubx.parser import autarky_parser
...
def parse(input_path: str) -> EnergySystem:
    ...
    logging.log(f"Starting parsing from input files at {input_path} ...",
                module=LOG_MODULE_STR)
    energy_system = _parse_self(input_path)
    _parse_modules(input_path, energy_system)
    elapsed = datetime.now() - start_time
    logging.log((f"Finished parsing. Elapsed time: "
...
def _parse_self(input_path: str) -> EnergySystem:
    ...
    stage_file_path = os.path.join(input_path, DIR_BASIC,
                                   stage_parser.FILENAME_STAGES)
    yaml_parser.check_file_exists(stage_file_path,
                                  stage_parser.FILETYPE_STAGES)
    stage_root_node = yaml_parser.parse(stage_file_path)
    # Empty stage file
    if stage_root_node is None:
        raise exceptions.MissingRootNodeException(stage_file_path,
                                                  module=LOG_MODULE_STR)
...
def _parse_modules(input_path: str, energy_system: EnergySystem) -> None:
    ...
    net_links, net_link_root_node = net_link_parser.parse(network_subpath,
                                                          stages)
    ...
    techs, tech_root_node = tech_parser.parse_primary(basic_subpath,
        energy_system, stages)
    conv_tech_parser.preprocess_in_ec_groups(tech_root_node, ec_root_node,
                                             techs)
    ...
    net_techs = net_tech_parser.parse_primary(network_subpath,
                                              energy_system, stages)
