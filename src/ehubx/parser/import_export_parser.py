...
def parse_exports(basic_subpath: str) -> Exports:
    ...
    for export_node in export_root_node:
        ...
        if len(dupe_tuples) > 0:
            msg = (
                "Overlap detected in export module: "
                + f"The (stage, hub, ec) tuples {dupe_tuples} occur in more "
                + "than one export entry"
            )
            raise exceptions.ParsingException(
                export_node.file_path, msg, module=LOG_MODULE_STR
            )
        for stage_id, hub_id, ec_id in id_tuples:
...
def _parse_import_tuple(
    s: StageId, h: HubId, e: EcId, node: yaml_parser.YamlNode, imports: Imports
) -> None:
    # tuple
    imports.add_tuple(s, h, e)
    # price
    price = yaml_parser.parse_optional_float_value_from_dict_node(node, YAMLKEY_PRICE)
    if price is not None:
        imports.set_price_def(s, h, e, price)
    # co2
    co2 = yaml_parser.parse_optional_float_value_from_dict_node(node, YAMLKEY_CO2)
    if co2 is not None:
        imports.set_co2_def(s, h, e, co2)
    # min
    imp_min = yaml_parser.parse_optional_float_value_from_dict_node(node, YAMLKEY_MIN)
    if imp_min is not None:
        imports.set_min_def(s, h, e, imp_min)
    # max
    imp_max = yaml_parser.parse_optional_float_value_from_dict_node(node, YAMLKEY_MAX)
...
def _parse_export_tuple(
    s: StageId, h: HubId, e: EcId, node: yaml_parser.YamlNode, exports: Exports
) -> None:
    # tuple
    ...
    # min
    exp_min = yaml_parser.parse_optional_float_value_from_dict_node(node, YAMLKEY_MIN)
    if exp_min is not None:
        exports.set_min_def(s, h, e, exp_min)
    # max
    exp_max = yaml_parser.parse_optional_float_value_from_dict_node(node, YAMLKEY_MAX)
...
def _parse_import_profiles(
    profile_path: str, stage_id: StageId, hub_id: HubId, ec_id: EcId, imports: Imports
) -> None:
    ...
    for s, h, e, key in df_profiles.columns:
        ...
        if key == YAMLKEY_PRICE:
            for t in df_profiles.index:
                imports.set_price(
                    stage_id, hub_id, ec_id, TimeId(t), df_profiles[s, h, e, key][t]
                )
        if key == YAMLKEY_CO2:
            for t in df_profiles.index:
                imports.set_co2(
...
def _parse_export_profiles(
    profile_path: str, stage_id: StageId, hub_id: HubId, ec_id: EcId, exports: Exports
) -> None:
    yaml_parser.check_file_exists(profile_path, FILETYPE_IMPEXPPROFILE)
    df_profiles = csv_parser.parse(
        profile_path,
