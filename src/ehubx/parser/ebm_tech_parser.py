...
def _parse_ebm_tech_primary(tech_node: yaml_parser.YamlDictNode,
                            stages: Stages, ebm_techs: EbmTechs) -> None:
    # tech_id
...
def _parse_hub_secondary(hub_node: yaml_parser.YamlDictNode, stages: Stages,
                         ebm_techs: EbmTechs) -> None:
    # id
...
def _parse_tech_secondary_profiles(ebm_params_node: yaml_parser.YamlNode,
    ...
    if profile_path is not None:
        profile_path = os.path.abspath(os.path.join(
            ebm_params_node.file_path, os.pardir, profile_path))
        ...
        for (s, h, x, profile_key) in df.columns:
            ...
            if profile_key == YAMLKEY_DEMANDNOMINAL:
                for t in df.index:
                    ebm_techs.set_demand_nominal(stage_id, hub_id, tech_id,
                        TimeId(t), df[s, h, x, profile_key][t])
            if profile_key == YAMLKEY_AVAILABILITY:
                for t in df.index:
                    ebm_techs.set_availability(stage_id, hub_id, tech_id,
                        TimeId(t), df[s, h, x, profile_key][t])
