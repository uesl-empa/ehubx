...
YAMLKEY_LASTINSTLYEAR = "last_instl_year"
...
def parse_primary(basic_subpath: str, energy_system: EnergySystem,
                  stages: Stages
                  ) -> Tuple[Techs, Optional[yaml_parser.YamlNode]]:
    # Create techs
    techs = Techs()
    # Parse file
...
def _preprocess_coupled_techs_primary(
        techs_node: yaml_parser.YamlListNode) -> None:
    for tech_node in techs_node:
        ...
        if coupling_params_node is None:
            continue
        tech_id = yaml_parser.parse_mandatory_str_value_from_dict_node(
            tech_node, YAMLKEY_TECHID)
        yaml_parser.check_node_type(coupling_params_node,
                                    yaml_parser.YamlNodeKind.DICT)
        # Get main node
        main_tech_id = yaml_parser.parse_mandatory_str_value_from_dict_node(
            coupling_params_node, YAMLKEY_MAINTECHID)
        if tech_id == main_tech_id:
            main_tech_id_path_str = (coupling_params_node[YAMLKEY_MAINTECHID]
                                     .node_path_as_str)
            msg = f"Coupled tech {tech_id} references itself as main_tech_id"
            raise exceptions.InvalidValueException(techs_node.file_path,
                main_tech_id_path_str, msg, module=LOG_MODULE_STR)
        main_tech_node = techs_node[main_tech_id]
        if main_tech_node is None:
            node_path_str = coupling_params_node.node_path_str + "|" + \
                f"{YAMLKEY_MAINTECHID}"
            invalidity_reason = f"Tech id {main_tech_id} was not found " + \
                "in techs list"
            raise exceptions.InvalidValueException(techs_node.file_path,
        ...
        if tech_node[YAMLKEY_TECHPARAMS] is not None:
            node_path_str = tech_node.node_path_as_str + "|" +  \
                YAMLKEY_TECHPARAMS
            invalidity_reason = f"Tech {tech_id} qualifies as a coupled " + \
        ...
        yaml_parser.check_node_type(tech_params_node,
                                    yaml_parser.YamlNodeKind.DICT)
        tech_node.add_dict_child(YAMLKEY_TECHPARAMS, tech_params_node.copy())


def _parse_tech_primary(tech_node: yaml_parser.YamlDictNode,
    ...
    tech_type = yaml_parser.parse_optional_str_value_from_dict_node(
        tech_node, YAMLKEY_TYPE)
    if tech_node is None:
        logging.log_warning(f"Tech {tech_id} does not have a type",
                            module=LOG_MODULE_STR)
    if tech_type is not None:
        if tech_type not in [tech_type.value for tech_type in TechType]:
            node_path_str = tech_node.node_path_as_str + "|" + YAMLKEY_TYPE
            invalidity_reason = "Not a known tech type"
            raise exceptions.InvalidValueException(tech_node.file_path,
                node_path_str, invalidity_reason, module=LOG_MODULE_STR)
    # tech_params
    _parse_tech_params_primary(tech_node, tech_id, energy_system, stages,
                               techs)
    # costs
...
def _parse_tech_params_primary(tech_node: yaml_parser.YamlDictNode,
                               tech_id: TechId, energy_system: EnergySystem,
                               stages: Stages, techs: Techs) -> None:
    # tech_params node
    tech_params_node = yaml_parser.get_mandatory_subnode_from_dict_node(
    ...
    lifetime = yaml_parser.parse_mandatory_int_value_from_dict_node(
        tech_params_node, YAMLKEY_LIFETIME)
    ...
    trl_dict = yaml_parser.parse_optional_yeardep_float_from_dict_node(
        tech_params_node, YAMLKEY_TRL, stages)
    if trl_dict is None:
...
def _parse_emissions(tech_node: yaml_parser.YamlDictNode, tech_id: TechId,
                     stages: Stages, techs: Techs) -> None:
...
def _parse_coupled_techs(tech_node: yaml_parser.YamlDictNode,
                         techs: Techs) -> None:
    # Tech id
    tech_id_str = yaml_parser.parse_mandatory_str_value_from_dict_node(
        tech_node, YAMLKEY_TECHID)
    ...
    yaml_parser.check_node_type(coupling_params_node,
                                yaml_parser.YamlNodeKind.DICT)
    # main_tech_id
    main_tech_id_str = yaml_parser.parse_mandatory_str_value_from_dict_node(
        coupling_params_node, YAMLKEY_MAINTECHID)
    main_tech_id = TechId(main_tech_id_str)
    techs.set_coupled_main_tech(tech_id, main_tech_id)
    # cap_factor
    cap_factor = yaml_parser.parse_mandatory_float_value_from_dict_node(
        coupling_params_node, YAMLKEY_CAPFACTOR)
...
def _parse_hub_secondary(hub_node: yaml_parser.YamlNode,
...
def _parse_allowed_techs(hub_node: yaml_parser.YamlNode,
                         tech_lists: Dict[str, List[TechId]],
                         hub_id: HubId, techs: Techs) -> Set[TechId]:
    ...
    for tech_list_id in allowed_tech_lists:
        if tech_list_id not in tech_lists:
            node_path_str = hub_node.node_path_as_str + "|" + \
                YAMLKEY_ALLOWEDTECHLISTS
            invalidity_reason = f"{tech_list_id} is not a known " + \
                f"{YAMLKEY_TECHLISTID}"
            raise exceptions.InvalidValueException(hub_node.file_path,
                node_path_str, invalidity_reason, module=LOG_MODULE_STR)
    ...
    for main_tech_id in allowed_techs.intersection(techs.coupled_main_techs):
        ...
        for sub_tech_id in sub_techs:
            if sub_tech_id not in allowed_techs:
                msg = (f"Added coupled sub tech {sub_tech_id} to allowed techs"
...
def _preprocess_coupled_techs_secondary(
        techs_node: yaml_parser.YamlNode, techs: Techs) -> None:
    yaml_parser.check_node_type(techs_node, yaml_parser.YamlNodeKind.LIST)
    for tech_id in techs.coupled_sub_techs:
        ...
        new_tech_params_node = yaml_parser.YamlDictNode(
            techs_node.file_path)
        # cap_init
        cap_init_main = yaml_parser.parse_optional_float_value_from_dict_node(
            main_tech_params_node, YAMLKEY_CAPINIT)
        if cap_init_main is not None:
            cap_init = techs.get_coupled_cap_factor(tech_id) * cap_init_main
            new_tech_params_node.add_child_value(YAMLKEY_CAPINIT, cap_init)
        # age_init
        age_init_main = yaml_parser.parse_optional_float_value_from_dict_node(
            main_tech_params_node, YAMLKEY_AGEINIT)
        if age_init_main is not None:
            age_init = age_init_main
            new_tech_params_node.add_child_value(YAMLKEY_AGEINIT, age_init)
        # last_instl_year
        last_instl_year_main = \
            yaml_parser.parse_optional_float_value_from_dict_node(
                main_tech_params_node, YAMLKEY_LASTINSTLYEAR)
        if last_instl_year_main is not None:
            last_instl_year = last_instl_year_main
            new_tech_params_node.add_child_value(YAMLKEY_LASTINSTLYEAR,
                                                 last_instl_year)
...
def _parse_tech_secondary(tech_node: yaml_parser.YamlDictNode,
                          hub_id: HubId, stages: Stages,
                          techs: Techs, allowed_techs: Set[TechId]) -> None:
    # tech_id
    tech_id_str = yaml_parser.parse_mandatory_str_value_from_dict_node(
        tech_node, YAMLKEY_TECHID)
    tech_id = TechId(tech_id_str)
    ...
    if tech_id not in allowed_techs:
        msg = (f"Parsing irregularity in file {tech_node.file_path}: "
    ...
    yaml_parser.check_node_type(tech_params_node,
                                yaml_parser.YamlNodeKind.DICT)
    # last_instl_year
    last_instl_year = yaml_parser.parse_optional_float_value_from_dict_node(
        tech_params_node, YAMLKEY_LASTINSTLYEAR)
    if last_instl_year is not None:
        techs.set_last_instl_year(hub_id, tech_id, last_instl_year)
    # cap_init
    cap_init = yaml_parser.parse_optional_float_value_from_dict_node(
        tech_params_node, YAMLKEY_CAPINIT)
    ...
    age_init = yaml_parser.parse_optional_float_value_from_dict_node(
        tech_params_node, YAMLKEY_AGEINIT)
    ...
    cap_min = yaml_parser.parse_optional_yeardep_float_from_dict_node(
        tech_params_node, YAMLKEY_CAPMIN, stages)
    ...
    cap_max = yaml_parser.parse_optional_yeardep_float_from_dict_node(
        tech_params_node, YAMLKEY_CAPMAX, stages)
    if cap_max is not None:
        for stage_id, value in cap_max.items():
            techs.set_cap_max(stage_id, hub_id, tech_id, value)


def _parse_tech_lists(hub_root_node: yaml_parser.YamlNode,
                      techs: Techs) -> Dict[str, List[TechId]]:
    tech_lists: Dict[str, List[TechId]] = {}
    ...
    for tech_list_node in tech_lists_node:
        tech_list_id = yaml_parser.parse_mandatory_str_value_from_dict_node(
        ...
        for tech_id_str in techs_in_list:
            ...
            if tech_id not in techs.ids:
                logging.log_warning(
                    ...
                    f"{tech_list_node.file_path}", module=LOG_MODULE_STR)
...
def _log(techs: Techs) -> None:
    ...
    for x in techs.ids:
        logging.log_file(f"  Tech {x}: Lifetime = {techs.get_lifetime(x)}",
                         print_time=False)
