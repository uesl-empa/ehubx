...
def parse_primary(
    basic_subpath: str, energy_system: EnergySystem, stages: Stages
...
def _parse_tech_primary(
    ...
    if tech_node is None:
        logging.log_warning(
            f"Tech {tech_id} does not have a type", module=LOG_MODULE_STR
        )
...
def _preprocess_coupled_techs_secondary(
    ...
    for tech_id in techs.coupled_sub_techs:
        ...
        # last_instl_year
        last_instl_year_main = yaml_parser.parse_optional_float_value_from_dict_node(
            main_tech_params_node, YAMLKEY_LASTINSTLYEAR
        )
        if last_instl_year_main is not None:
            last_instl_year = last_instl_year_main
            new_tech_params_node.add_child_value(YAMLKEY_LASTINSTLYEAR, last_instl_year)
