...
from ehubx.model.energy_system_model import (
    ...
    configure_energy_system_features,
)
...
def _set_weighted_objective(
    ...
    configure_energy_system_features(model, enabled=allow_autonomy)
    if model is None:
        raise exceptions.EhubXException(
...
def _get_objective_value(model: Model, obj_type: ObjectiveType) -> float:
    ...
    if obj_type == ObjectiveType.AUTONOMY:
        obj_value = getattr(model, VAR_SYSTEMAUTONOMY).value
...
def set_objective(model: Model, objective_type: ObjectiveType) -> None:
    ...
    configure_energy_system_features(
        model, enabled=(objective_type == ObjectiveType.AUTONOMY)
    )
...
def _set_threshold_constraint(
    ...
    configure_energy_system_features(
        model, enabled=(obj_type == ObjectiveType.AUTONOMY)
    )
    # Cost minimization
    if obj_type == ObjectiveType.COST:
