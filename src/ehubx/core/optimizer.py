...
def _get_objective_value(model: Model, obj_type: ObjectiveType) -> float:
    ...
    if obj_type == ObjectiveType.AUTARKY:
        obj_value = getattr(model, VAR_SYSTEMAUTARKY).value
...
def set_objective(model: Model, objective_type: ObjectiveType) -> None:
    ...
    # Autarky maximization (include small amount of cost minimization as
    ...
    if objective_type == ObjectiveType.AUTARKY:
        ...
        var_cost = getattr(model, VAR_SYSTEMCOST)
        var_autarky = getattr(model, VAR_SYSTEMAUTARKY)
        obj = Objective(expr=(-var_autarky + EPS_WEIGHTEDSUM * var_cost))
        obj_name = "O_SystemAutarkyWeak"
        if model.find_component(obj_name) is not None:
            model.del_component(obj_name)
        model.add_component(obj_name, obj)
        obj.activate()
        msg += "Autarky maximization"
    logging.log(msg, module=LOG_MODULE_STR)
...
def _set_threshold_constraint(model: Model, obj_type: ObjectiveType,
    ...
    if obj_type == ObjectiveType.AUTARKY:
        ...
        sub_str = f"Autarky minimum (threshold = {threshold})"
