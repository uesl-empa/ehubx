...
class Imports:
    ...
    def time_series(self) -> List[Tuple[TimeSeriesKind, StageId,
        ...
        for (s, h, e), series in self._price.items():
            if series.has_values:
                all_series.append((TimeSeriesKind.IMPORTPRICE, s,
                                   (h.key, e.key), series))
        # min
        for (s, h, e), series in self._min.items():
            if series.has_values:
                all_series.append((TimeSeriesKind.IMPORTMIN, s,
                                   (h.key, e.key), series))
        # max
        for (s, h, e), series in self._max.items():
            if series.has_values:
                all_series.append((TimeSeriesKind.IMPORTMAX, s,
                                   (h.key, e.key), series))
        # co2
        for (s, h, e), series in self._co2.items():
            if series.has_values:
                all_series.append((TimeSeriesKind.IMPORTCO2, s,
                                   (h.key, e.key), series))
        return all_series
            ...
            if imp_min.has_values or imp_max.has_values:
                for t in times.ids:
                    if imp_min.get_value(t) > imp_max.get_value(t):
                        msg = (f"{imp_min.get_value(t)} = min[{s}, {h}, {e}]"
                               f"[{t}] > max[{s}, {h}, {e}][{t}] = "
    ...
    def _check_ids(self, s: StageId, h: HubId, e: EcId,
        ...
        if (s, h, e) not in self._tuples:
            msg = (f"Encountered tuple ({s}, {h}, {e}) which is not an import "
