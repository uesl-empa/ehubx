...
class EnergySystem:
    ...
    def num_times_horizon(self) -> int:
        ...
        if self._num_times_horizon is None:
            raise exceptions.MissingValueException(
                ExceptionKey.NUMTIMESHORIZON_GET.value,
    ...
    def deactivate_imports(self) -> None:
        ...
        if self.imports_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated import module. "
                "Skipping ...",
                module=LOG_MODULE_STR)
            return
    ...
    def deactivate_demands(self) -> None:
        ...
        if self.demands_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated "
                "demand module. Skipping ...",
                module=LOG_MODULE_STR)
            return
        if not self.load_shedding_deactivated:
            logging.log(("Triggering load shedding module deactivation due to "
                         " demand module deactivation ..."),
                        module=LOG_MODULE_STR)
            self.deactivate_load_shedding()
        if not self.load_shifting_deactivated:
            logging.log(("Triggering load shifting module deactivation due to "
                         "demand module deactivation ..."),
                        module=LOG_MODULE_STR)
    ...
    def deactivate_stor_techs(self) -> None:
        ...
        if self.stor_techs_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated "
                "storage tech module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def deactivate_ebm_techs(self) -> None:
        ...
        if self.ebm_techs_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated "
                "EBM tech module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def deactivate_hp_techs(self) -> None:
        ...
        if self.hp_techs_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated "
                "heat pump tech module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def deactivate_ates_techs(self) -> None:
        ...
        if self.ates_techs_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated "
                "ATES tech module. Skipping ...", module=LOG_MODULE_STR)
            return
        # Create backups of ates_techs and the ids in techs
    ...
    def deactivate_conv_techs(self) -> None:
        ...
        if self.conv_techs_deactivated:
            logging.log_warning(
                "Tried to deactivate already deactivated "
                "conversion tech module. Skipping ...", module=LOG_MODULE_STR)
            return
        ...
        for x in self.conv_techs.ids.copy():
            if x not in self._deactivated_conv_techs.ids:
                copy_over_conv_tech(x, self.conv_techs,
                                    self._deactivated_conv_techs, self.stages,
                                    self.hubs, self.times)
            if x not in self._deactivated_techs.ids:
                copy_over_tech(x, self.techs, self._deactivated_techs,
                               self.stages, self.hubs)
        ...
        logging.log("Deactivated conversion tech module",
                    module=LOG_MODULE_STR)
    ...
    def reactivate_imports(self) -> None:
        ...
        if not self.imports_deactivated:
            logging.log_warning(
                ...
                "import module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def reactivate_exports(self) -> None:
        ...
        if not self.exports_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "export module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def reactivate_demands(self) -> None:
        ...
        if not self.demands_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "demand module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def reactivate_load_shedding(self) -> None:
        ...
        if not self.load_shedding_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "load shedding module. Skipping ...", module=LOG_MODULE_STR)
            return
        if self.demands_deactivated:
            logging.log(
                "Triggering demand module reactivation due to "
                "load shedding module reactivation ...", module=LOG_MODULE_STR)
            self.reactivate_demands()
        deactivated_load_shedding = copy.deepcopy(
            self._deactivated_load_shedding)
    ...
    def reactivate_load_shifting(self) -> None:
        ...
        if not self.load_shifting_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "load shifting module. Skipping ...", module=LOG_MODULE_STR)
            return
        if self.demands_deactivated:
            logging.log(
                "Triggering demand module reactivation due to "
                "load shifting module reactivation ...", module=LOG_MODULE_STR)
            self.reactivate_demands()
        deactivated_load_shifting = copy.deepcopy(
            self._deactivated_load_shifting)
    ...
    def reactivate_network(self) -> None:
        ...
        if not self.network_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "network module. Skipping ...", module=LOG_MODULE_STR)
            return
    ...
    def reactivate_techs(self) -> None:
        ...
        if not self.techs_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "tech module. Skipping ...", module=LOG_MODULE_STR)
            return
        # Reactivate module
    ...
    def reactivate_stor_techs(self) -> None:
        ...
        if self.techs_deactivated:
            logging.log(
                ...
                "storage tech module reactivation ...", module=LOG_MODULE_STR)
            self.reactivate_techs()
        else:
            # Manually reactivate the techs
            deactivated_stor_techs = copy.deepcopy(
                self._deactivated_stor_techs)
            assert deactivated_stor_techs is not None
            for x in deactivated_stor_techs.ids.copy():
                if x not in self.techs.ids:
                    copy_over_tech(x, self._deactivated_techs, self.techs,
                                self.stages, self.hubs)
    ...
    def reactivate_ebm_techs(self) -> None:
        ...
        if self.techs_deactivated:
            logging.log(
                "Triggering tech module reactivation due to "
                "EBM tech module reactivation ...", module=LOG_MODULE_STR)
            self.reactivate_techs()
        else:
            # Manually reactivate the techs
    ...
    def reactivate_ates_techs(self) -> None:
        ...
        if self.techs_deactivated:
            ...
            # Manually reactivate the techs
            deactivated_ates_techs = copy.deepcopy(
                self._deactivated_ates_techs)
    ...
    def reactivate_conv_techs(self) -> None:
        ...
        logging.log("Reactivated conversion tech module",
                    module=LOG_MODULE_STR)
    ...
    def reactivate_solar_techs(self) -> None:
        ...
        if not self.solar_techs_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "solar tech module. Skipping ...", module=LOG_MODULE_STR)
            return
        # Trigger conv_tech module reactivation
        if self.conv_techs_deactivated:
            logging.log(
                "Triggering conversion tech module reactivation due "
                "to solar tech module reactivation ...", module=LOG_MODULE_STR)
            ...
            for x in deactivated_solar_techs.ids:
                if x not in self.conv_techs.ids:
                    copy_over_conv_tech(x, self._deactivated_conv_techs,
                                        self.conv_techs, self.stages,
                                        self.hubs, self.times)
    ...
    def reactivate_wind_techs(self) -> None:
        ...
        if not self.wind_techs_deactivated:
            logging.log_warning(
                "Tried to reactivate not-deactivated "
                "wind tech module. Skipping ...", module=LOG_MODULE_STR)
            return
        # Trigger conv_tech module reactivation
        if self.conv_techs_deactivated:
            logging.log(
                "Triggering conversion tech module reactivation due "
                "to wind tech module reactivation ...", module=LOG_MODULE_STR)
            self.reactivate_conv_techs()
        else:
            # Manually reactivate the conv_techs and techs
    ...
    def validate(self) -> None:
        ...
        logging.log(f"Finished data validation. Elapsed time: "
    ...
    def _validate_interest_rate_def(self) -> None:
        ...
        if self._interest_rate_def < 0:
            ...
            raise exceptions.DataException(
                ExceptionKey.INTERESTRATEDEF_VAL.value, [], msg,
                module=LOG_MODULE_STR)
    ...
    def _validate_num_times_horizon(self) -> None:
        ...
        if self._num_times_horizon <= 0:
            ...
            raise exceptions.DataException(
                ExceptionKey.NUMTIMESHORIZON_VAL.value, [], msg,
                module=LOG_MODULE_STR)
    ...
    def _validate_modules(self) -> None:
        ...
        self.load_shedding.validate(self.stages, self.hubs, self.ecs,
                                    self.demands, self.times)
        self.load_shifting.validate(self.stages, self.hubs, self.ecs,
                                    self.demands, self.times)
        self.techs.validate(self.stages, self.hubs)
        self.stor_techs.validate(self.stages, self.hubs, self.ecs, self.techs)
        self.ebm_techs.validate(self.stages, self.hubs, self.techs, self.ecs,
                                self.times)
        self.conv_techs.validate(self.stages, self.hubs, self.ecs, self.techs,
                                 self.times)
        self.solar_data.validate(self.stages, self.hubs, self.ecs,
                                 self.times)
        self.solar_techs.validate(self.stages, self.hubs, self.imports,
                                  self.techs, self.conv_techs, self.solar_data)
        self.wind_data.validate(self.stages, self.hubs, self.ecs, self.times)
        self.wind_techs.validate(self.stages, self.hubs, self.imports,
                                 self.techs, self.conv_techs, self.wind_data)
        self.hp_techs.validate(self.stages, self.hubs, self.ecs, self.techs,
                               self.times)
