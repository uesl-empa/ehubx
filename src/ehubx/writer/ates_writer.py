...
def _format_tech(
    ...
    # ATES area and well pairs per schedule
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            if h not in energy_system.techs.get_allowed_hubs(x):
                continue
            for i in energy_system.ates_data.get_schedule_ids(h):
                cap = value(var_cap[s.key, h.key, x.key, i.key], exception=False)
                if cap is None:
                    continue
                add_to_df_st(
                    df_st,
                ...
                area_calc_method = (
                    energy_system.ates_techs.get_well_pair_area_calc_method(x)
                )
                well_pair_area = energy_system.ates_techs.calc_area_per_well_pair(
                    therm_rad_warm, therm_rad_cold, area_calc_method
                )
                num_well_pairs = cap / well_pair_area
                add_to_df_st(
                    df_st,
    ...
    for s_, h_, x_, i_ in getattr(model, ates_tech_model.SET_ATESTECHTUPLESCHEDULE):
        ...
        for t in energy_system.times.ids:
            elec.set_value(
                t, value(var_elec[s_, h_, x_, i_, t.key_as_int], exception=False)
            )
        add_to_df_ts_hor(
            df_ts_hor,
            energy_system.times,
    ...
    for s_, h_, x_, i_ in getattr(model, ates_tech_model.SET_ATESTECHTUPLESCHEDULE):
        ...
        for t in energy_system.times.ids:
            heat.set_value(
                t, value(var_heat[s_, h_, x_, i_, t.key_as_int], exception=False)
            )
        add_to_df_ts_hor(
            df_ts_hor,
            energy_system.times,
    ...
    for s_, h_, x_, i_ in getattr(model, ates_tech_model.SET_ATESTECHTUPLESCHEDULE):
        ...
        for t in energy_system.times.ids:
            cool.set_value(
                t, value(var_cool[s_, h_, x_, i_, t.key_as_int], exception=False)
            )
        add_to_df_ts_hor(
            df_ts_hor,
            energy_system.times,
    ...
    for s_, h_, x_, e_ in getattr(model, ates_tech_model.SET_ATESTECHIN):
        ...
        for t in energy_system.times.ids:
            ates_in.set_value(
                t, value(var_in[s_, h_, x_, e_, t.key_as_int], exception=False)
            )
        add_to_df_ts_hor(
            df_ts_hor,
            energy_system.times,
    ...
    for s_, h_, x_, e_ in getattr(model, ates_tech_model.SET_ATESTECHOUT):
        ...
        for t in energy_system.times.ids:
            ates_out.set_value(
                t, value(var_out[s_, h_, x_, e_, t.key_as_int], exception=False)
            )
        add_to_df_ts_hor(
            df_ts_hor,
            energy_system.times,
