...
ENTRY_LOADSHIFTINGCOSTENERGY: str = (
    f"Load shifting energy cost ({load_shifting_model.VAR_LOADHSHIFTINGCOSTENERGY})"
)
"""Entry name for load shifting energy cost variable in result files"""

ENTRY_LOADSHIFTINGCOSTPEAK: str = (
    f"Load shifting peak cost ({load_shifting_model.VAR_LOADHSHIFTINGCOSTPEAK})"
)
"""Entry name for load shifting peak cost variable in result files"""

ENTRY_LOADSHIFTINGCOSTFIX: str = (
    f"Load shifting fix cost ({load_shifting_model.VAR_LOADHSHIFTINGCOSTFIX})"
)
"""Entry name for load shifting fixed cost variable in result files"""

ENTRY_LOADSHIFTINGCOSTTOTAL: str = (
    f"Load shifting total cost ({load_shifting_model.VAR_LOADHSHIFTINGCOSTTOTAL})"
)
...
def format_all(
    ...
    for ls in energy_system.load_shifting.ids:
        for s, h, e in energy_system.load_shifting.get_tuples(ls):
            _format_ls_and_tuple(energy_system, ls, s, h, e, df_st, df_ts_hor, df_ts_cl)

    # Tuple-specific values
    for s_, h_, e_ in getattr(model, load_shifting_model.SET_LOADSHIFTINGTUPLE):
        _format_tuple(
            energy_system,
            model,
            StageId(s_),
...
def _format_tuple(
    ...
    # YLoadShifting
    if (s.key, h.key, e.key) in getattr(
        model, load_shifting_model.SET_LOADSHIFTINGTUPLEFIX
    ):
        var = getattr(model, load_shifting_model.VAR_YLOADHSHIFTING)
        y_load_shifting = TimeSeries()
        for t in energy_system.times.ids:
            y_load_shifting.set_value(
                t, value(var[s.key, h.key, e.key, t.key_as_int], exception=False)
            )
        add_to_df_ts_cl(
            df_ts_hor,
...
def write_time_series(load_shifting: LoadShifting, times: Times, dir_path: str) -> None:
    """
    ...
    if data:
        ...
        df.columns.names = [HeaderId.LOADSHIFTID.value, HeaderId.PROFILEKEY.value]
        df.index += 1
