...
from ehubx.model import demand_model, energy_system_model
...
ENTRY_BIGMGENERIC: str = f"Generic BigM parameter ({demand_model.PAR_BIGMGENERIC})"
"""Entry name for generic, demand-based bigM parameter inr esult files"""

ENTRY_DEMANDSUPPLY: str = f"Demand supply ({energy_system_model.VAR_DEMANDSUPPLY})"
...
def format_all(
    ...
    add_to_df_st(df_st, ENTRY_BIGMGENERIC, big_m_generic, source=SOURCE, in_res="input")
    ...
    # Child modules
    load_shedding_writer.format_all(energy_system, model, df_st, df_ts_hor, df_ts_cl)
    load_shifting_writer.format_all(energy_system, model, df_st, df_ts_hor, df_ts_cl)

    # Remove unused columns
    cols_to_drop_st = [COL_TECH, COL_NETLINK, COL_NETLINKDIR, COL_NETTECH, COL_WINDPARK]
...
def write_input_time_series(demands: Demands, times: Times, dir_path: str) -> None:
    """
