...
from ehubx.model import autarky_model, energy_system_model
...
ENTRY_AUTARKYIMPCROSS: str = f"Cross-imports ({autarky_model.VAR_AUTARKYIMPCROSS})"
...
def _format_all_system(
    ...
    add_to_df_st(
        df_st, ENTRY_TRLTHRESHOLD, trl_threshold, source=SOURCE, in_res="input"
    )

    # Number of horizon timesteps
    num_ts_hor = energy_system.num_times_horizon
    add_to_df_st(
        df_st, ENTRY_NUMTIMESHORIZON, num_ts_hor, source=SOURCE, in_res="input"
    )
    ...
    if energy_system.times.is_clustered:
        ...
        add_to_df_st(
            df_st, ENTRY_NUMTIMESCLUSTERED, num_ts_cl, source=SOURCE, in_res="result"
        )
    ...
    add_to_df_st(
        df_st,
        ENTRY_AUTARKYCALCULATIONMETHOD,
        aut_calc_method.value,
        source=SOURCE_AUTARKY,
        in_res="input",
    )
    ...
    add_to_df_st(
        df_st, ENTRY_SYSTEMCOST, cost, unit="CHF", source=SOURCE, in_res="result"
    )
