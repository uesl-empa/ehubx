...
from ehubx.parser.load_shedding_parser import YAMLKEY_MAXABS, YAMLKEY_MAXREL, \
    YAMLKEY_ENERGYCOST
...
ENTRY_LOADSHEDDING: str = \
    f"Load shedding ({load_shedding_model.VAR_LOADSHEDDING})"
...
ENTRY_LOADSHEDDINGCOSTTOTAL: str = \
    ("Total load shedding cost "
     f"({load_shedding_model.VAR_LOADHSHEDDINGCOSTTOTAL})")
...
def format_all(energy_system: EnergySystem, model: Model, df_st: pd.DataFrame,
    ...
    add_to_df_st(df_st, ENTRY_LOADSHEDDINGCOSTTOTAL, total_load_shedding_cost,
                 unit="CHF", source=SOURCE, in_res="result")

    # Tuple-specific values
    for (s, h, e) in energy_system.load_shedding.get_enabled_tuples(
            energy_system.stages, energy_system.hubs, energy_system.ecs,
...
def _format_tuple(energy_system: EnergySystem, model: Model, s: StageId,
    ...
    if max_abs.has_values:
        add_to_df_ts_cl(df_ts_hor, df_ts_cl,
            energy_system.times, ENTRY_MAXABS, max_abs, unit="kW",
    ...
    if not max_abs.has_values:
        ...
        add_to_df_st(df_st, ENTRY_MAXABS, max_abs_def, unit="kW", stage=s.key,
    ...
    if max_rel.has_values:
        add_to_df_ts_cl(df_ts_hor, df_ts_cl,
            energy_system.times, ENTRY_MAXREL, max_rel, unit="1",
            stage=s.key, hub=h.key, ec=e.key, source=SOURCE, in_res="input")
    if not max_rel.has_values:
        max_rel_def = max_rel.def_value
        assert max_rel_def is not None
        add_to_df_st(df_st, ENTRY_MAXREL, max_rel_def, unit="1", stage=s.key,
                     hub=h.key, ec=e.key, source=SOURCE, in_res="input")
    ...
    if energy_cost.has_values:
        add_to_df_ts_cl(df_ts_hor, df_ts_cl,
            energy_system.times, ENTRY_ENERGYCOST, energy_cost, unit="CHF/kW",
            stage=s.key, hub=h.key, ec=e.key, source=SOURCE, in_res="input")
    if not energy_cost.has_values:
        energy_cost_def = energy_cost.def_value
        assert energy_cost_def is not None
        add_to_df_st(df_st, ENTRY_ENERGYCOST, energy_cost_def, unit="CHF/kW",
    ...
    for t in energy_system.times.ids:
        load_shedding.set_value(t, value(var[s.key, h.key, e.key,
                                             t.key_as_int],
                                         exception=False))
    add_to_df_ts_cl(df_ts_hor, df_ts_cl, energy_system.times,
        ENTRY_LOADSHEDDING, load_shedding, unit="kW", stage=s.key, hub=h.key,
    ...
    add_to_df_st(df_st, ENTRY_LOADSHEDDINGCOST, load_shedding_cost, unit="CHF",
                 stage=s.key, hub=h.key, ec=e.key, source=SOURCE,
                 in_res="result")


def write_data_time_series(load_shedding: LoadShedding, times: Times,
                           dir_path: str) -> None:
    """
