...
from ehubx.writer.common_writer import FileGranularity, init_df_st, \
    init_df_ts_hor, init_df_ts_cl, \
    add_to_df_st, COL_TECH, COL_SOURCE, COL_NETLINK, COL_NETLINKDIR, \
    COL_NETTECH, COL_LOADSHIFT
from ehubx.writer import conv_tech_writer
from ehubx.writer import stor_tech_writer
...
ENTRY_TECHCAPINSTL: str = \
    f"Tech capacity installation ({tech_model.VAR_TECHCAPINSTL})"
...
ENTRY_TECHCOSTCAPEX: str = \
    f"CAPEX installation costs ({tech_model.VAR_TECHCOSTCAPEX})"
"""Entry name for tech CAPEX cost variable in result files"""

ENTRY_TECHCOSTOPEXCAP: str = \
    f"OPEX capacity costs ({tech_model.VAR_TECHCOSTOPEXCAP})"
...
def format_all(energy_system: EnergySystem, model: Model, dir_path: str,
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        if co2_total:
            add_to_df_st(df_st, ENTRY_TECHCO2TOTAL, co2_total, unit="kg",
    ...
    conv_tech_writer.format_all(energy_system, model, df_st, df_ts_hor,
                                df_ts_cl)
    stor_tech_writer.format_all(energy_system, model, df_st, df_ts_hor,
                                df_ts_cl)
    hp_tech_writer.format_all(energy_system, model, df_st, df_ts_hor, df_ts_cl)
    ebm_tech_writer.format_all(energy_system, model, df_st, df_ts_hor,
                               df_ts_cl)
...
def _format_tech(energy_system: EnergySystem, model: Model, x: TechId,
    ...
    cap_unit: str = "CAP"
    if (x in energy_system.conv_techs.ids
            or x in energy_system.hp_techs.ids):
        cap_unit = "kW"
    if (x in energy_system.solar_techs.ids
            or x in energy_system.wind_techs.ids
            or x in energy_system.ates_techs.ids):
        cap_unit = "m^2"
    if (x in energy_system.stor_techs.ids
            or x in energy_system.ebm_techs.ids):
        cap_unit = "kWh"
    ...
    for h in energy_system.hubs.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_ALLOWEDINHUB, allowed_in_hub, hub=h.key,
                     tech=x.key, source=SOURCE, in_res="input")

    # Lifetime
    lifetime = energy_system.techs.get_lifetime(x)
    add_to_df_st(df_st, ENTRY_LIFETIME, lifetime, unit="a", tech=x.key,
                 source=SOURCE, in_res="input")

    # Interest rate
    interest_rate = energy_system.techs.get_interest_rate(x)
    add_to_df_st(df_st, ENTRY_INTERESTRATE, interest_rate, tech=x.key,
                 source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_UNITCAPMIN, unit_cap_min, unit=cap_unit,
                     stage=s.key, tech=x.key, source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_ONETIMECAPEX, one_time_capex, unit="CHF",
                     stage=s.key, tech=x.key, source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_CAPEXPERCAP, capex_per_cap,
                     unit=f"CHF/{cap_unit}", stage=s.key, tech=x.key,
                     source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_ONETIMEOPEX, one_time_opex, unit="CHF",
                     stage=s.key, tech=x.key, source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_OPEXPERCAP, opex_per_cap,
                     unit=f"CHF/{cap_unit}", stage=s.key,
                     tech=x.key, source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_CO2PERCAP, co2_per_cap,
                     unit=f"CHF/{cap_unit}", stage=s.key, tech=x.key,
                     source=SOURCE, in_res="input")
    ...
    for h in energy_system.hubs.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_LASTINSTLYEAR, last_instl_year,
    ...
    for h in energy_system.hubs.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_CAPINIT, cap_init, unit=cap_unit,
                     hub=h.key, tech=x.key, source=SOURCE, in_res="input")
    ...
    for h in energy_system.hubs.ids_in_order:
        ...
        add_to_df_st(df_st, ENTRY_AGEINIT, age_init, unit="a", hub=h.key,
                     tech=x.key, source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            add_to_df_st(df_st, ENTRY_CAPMIN, cap_min, unit=cap_unit,
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            add_to_df_st(df_st, ENTRY_CAPMAX, cap_max, unit=cap_unit,
                         stage=s.key, hub=h.key, tech=x.key,
                         source=SOURCE, in_res="input")
    ...
    if x in energy_system.techs.coupled_sub_techs:
        ...
        add_to_df_st(df_st, ENTRY_COUPLEDMAINTECH, coupled_main_tech.key,
                     tech=x.key, source=SOURCE, in_res="input")

    # coupled_cap_factor
    if x in energy_system.techs.coupled_sub_techs:
        coupled_cap_factor = energy_system.techs.get_coupled_cap_factor(x)
        add_to_df_st(df_st, ENTRY_COUPLEDCAPFACTOR, coupled_cap_factor,
                     unit="1", tech=x.key, source=SOURCE, in_res="input")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            if cap:
                add_to_df_st(df_st, ENTRY_TECHCAP, cap, unit=cap_unit,
                             stage=s.key, hub=h.key, tech=x.key,
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            if cap_instl:
                add_to_df_st(df_st, ENTRY_TECHCAPINSTL, cap_instl,
                             unit=cap_unit, stage=s.key, hub=h.key,
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            y_cap_instl = value(var[s.key, h.key, x.key],
                                exception=False)
            if y_cap_instl:
                add_to_df_st(df_st, ENTRY_YTECHCAPINSTL, y_cap_instl,
                             stage=s.key, hub=h.key, tech=x.key,
                             source=SOURCE, in_res="result")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            y_tech_used = value(var[s.key, h.key, x.key],
                                exception=False)
            if y_tech_used:
                add_to_df_st(df_st, ENTRY_YTECHUSED, y_tech_used,
                             stage=s.key, hub=h.key, tech=x.key,
                             source=SOURCE, in_res="result")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            if cost_capex:
                add_to_df_st(df_st, ENTRY_TECHCOSTCAPEX, cost_capex,
                             unit="CHF", stage=s.key, hub=h.key,
                             tech=x.key, source=SOURCE, in_res="result")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            cost_opex_cap = value(var[s.key, h.key, x.key],
                                exception=False)
            if cost_opex_cap:
                add_to_df_st(df_st, ENTRY_TECHCOSTOPEXCAP, cost_opex_cap,
                             unit="CHF", stage=s.key, hub=h.key,
                             tech=x.key, source=SOURCE, in_res="result")
    ...
    add_to_df_st(df_st, ENTRY_TECHCOSTTOTAL, cost_total, unit="CHF",
                 tech=x.key, source=SOURCE, in_res="result")
    ...
    for s in energy_system.stages.ids_in_order:
        ...
        for h in energy_system.hubs.ids_in_order:
            ...
            if co2_instl:
                add_to_df_st(df_st, ENTRY_TECHCO2INSTL, co2_instl,
                             unit="kg", stage=s.key, hub=h.key, tech=x.key,
                             source=SOURCE, in_res="result")
...
def _format_file_granularity(df_st: pd.DataFrame, df_ts_hor: pd.DataFrame,
    ...
    if file_granularity == FileGranularity.DEFAULT:
        ...
        for x in ids_ts_hor:
            ...
            df_ts_hor_cur = df_ts_hor.xs(x, axis=1, level=COL_TECH,
                                         drop_level=False)
        ...
        if df_ts_cl is not None:
            ...
            for x in ids_ts_cl:
                ...
                df_ts_cl_cur = df_ts_cl.xs(x, axis=1, level=COL_TECH,
                                           drop_level=False)
    ...
    if file_granularity == FileGranularity.MAX:
        ...
        # Horizon time files
        ids_ts_hor = df_ts_hor.columns.to_frame(index=False)[[COL_TECH,
                                                              COL_SOURCE]]
        for (x, source) in ids_ts_hor.itertuples(index=False, name=None):
            ...
            filename_ts_hor = os.path.join(dir_path, f"{filename_ts_hor}.csv")
            df_ts_hor_cur = df_ts_hor.xs((x, source), axis=1,
                                         level=(COL_TECH, COL_SOURCE),
                                         drop_level=False)
            if len(df_ts_hor_cur) > 0:
                dfs.append((df_ts_hor_cur, filename_ts_hor))
        ...
        # Clustered time files
        if df_ts_cl:
            ids_ts_cl = df_ts_cl.columns.to_frame(index=False)[[COL_TECH,
                                                                COL_SOURCE]]
            for (x, source) in ids_ts_cl.itertuples(index=False, name=None):
                ...
                filename_ts_cl = os.path.join(dir_path,
                                              f"{filename_ts_cl}.csv")
                df_ts_cl_cur = df_ts_cl.xs((x, source), axis=1,
                                           level=(COL_TECH, COL_SOURCE),
                                           drop_level=False)
                if len(df_ts_hor_cur) > 0:
                    dfs.append((df_ts_cl_cur, filename_ts_cl))
